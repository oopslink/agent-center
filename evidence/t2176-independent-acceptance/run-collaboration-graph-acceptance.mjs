...
const REPO = process.env.REPO_ROOT || resolve(new URL("../../..", import.meta.url).pathname);
...
const CANDIDATE = process.env.CANDIDATE_SHA || "1e14098b213d97087c3aa2fbda9a90f065c74a3e";
const OUT = process.env.OUT_DIR || resolve(process.cwd(), "evidence/t2176-independent-acceptance/raw");
const REPORT_OUT = process.env.REPORT_OUT || resolve(process.cwd(), "evidence/t2176-independent-acceptance/REPORT.md");
const BIN = process.env.BIN_PATH || resolve(REPO, "bin/agent-center");
...
async function main() {
  ...
  const installRoot = await mkdtemp(join(tmpdir(), "t2176-independent-"));
...
`, "utf8");
  ...
  const server = spawn(BIN, ["server", "--config", configPath], {
    ...
    env: { ...process.env, AGENT_CENTER_INVOCATION_ID: "t2176-independent-ui" },
  });
  ...
  try {
    ...
    const ownerName = `t2176-owner-${Date.now().toString(36)}`;
    const passcode = "T2176Pass1!";
    const signup = await json(seedReq.post(apiURL + "/auth/signup", {
      data: {
        display_name: ownerName,
        passcode,
        organization_name: "T2176 Independent Org",
    ...
    const worker = await json(seedReq.post(`${org}/admintoken/mint-enroll`, { data: { name: "t2176-worker" } }), "mint worker");
    ...
    const agentA = await json(seedReq.post(`${org}/members/agent`, { data: {
      display_name: "Atlas agent", cli: "codex", model: "gpt-t2176", worker_id: worker.worker_id,
    } }), "agent A");
    const agentB = await json(seedReq.post(`${org}/members/agent`, { data: {
      display_name: "Beacon agent", cli: "codex", model: "gpt-t2176", worker_id: worker.worker_id,
    } }), "agent B");
    ...
    const projectFilter5xx = network.filter((r) => r.status >= 500 && r.url.includes("/insights/collaboration-effects") && r.url.includes("project_id="));
    ...
    gate(2, "No-query first screen non-empty 117+/250+ organization graph", !orgURL.includes("?") && orgView.nodes > 0 && orgView.edges > 0 && graphJSON.graph.nodes.length >= 117 && graphJSON.graph.edges.length >= 250 ? "PASS" : "FAIL",
    ...
    gate(4, "Search/filter and clear restores full graph without 503", filteredURL.includes(`project_id=${encodeURIComponent(projectID)}`) && !clearedURL.includes("?") && projectFilter5xx.length === 0 ? "PASS" : "FAIL",
      `filtered=${filteredURL}; cleared=${clearedURL}; project_filter_5xx=${projectFilter5xx.length}${projectFilter5xx[0] ? `; first_5xx=${projectFilter5xx[0].status} ${projectFilter5xx[0].url}` : ""}`,
      `Sign in, open /organizations/${slug}/insights/collaboration, choose project ${projectID}; observe 503 on /api/orgs/${slug}/insights/collaboration-effects?limit=100&project_id=${projectID}.`);
...
function renderReport(verdict, provenance, rows, perf) {
  const lines = [];
  lines.push("# T2176 Independent Collaboration Graph UI Acceptance");
  lines.push("");
  ...
  lines.push("- `raw/01-authenticated-projects.png`");
  lines.push("- `raw/02-organization-graph.png`");
  lines.push("- `raw/03-direct-refresh.png`");
  lines.push("- `raw/04-filtered-project.png`");
  lines.push("- `raw/05-agent-focus.png`");
  lines.push("- `raw/06-evidence-drawer.png`");
  lines.push("- `raw/network.har`, `raw/network.json`, `raw/console.json`, `raw/server.log`");
  lines.push("- `raw/api-org-graph.json`, `raw/api-lod-cluster.json`, `raw/api-evidence.json`, `raw/verdict.json`");
  lines.push("- `build.log`, `e2e-pnpm-install.log`, `acceptance-run.log`");
  lines.push("");
