...
export default function InsightCollaboration(): React.ReactElement {
  ...
  const [loadedGraph, setLoadedGraph] = useState<CollaborationGraphResponse | null>(null);
  const filters = filtersFromParams(params, loadedCursor);
  const query = useCollaborationEffects(filters);
  const graphData = loadedGraph ?? query.data ?? null;
  const selectedEffect = graphData?.effects.find((item) => item.effect_id === selectedEffectId) ?? null;

  useEffect(() => {
    if (!query.data) return;
    setLoadedGraph((prev) => loadedCursor ? mergeGraphPages(prev, query.data) : query.data);
  }, [loadedCursor, query.data]);
      ...
      {graphData ? (
        <>
          <OrganizationGraph
            ...
            effects={graphData.effects}
            selectedEffectId={selectedEffectId}
            focusedNode={focusedNode}
            search={search}
            truncated={Boolean(graphData.truncated || graphData.next_cursor)}
            onSelectEffect={setSelectedEffectId}
            onFocusNode={setFocusedNode}
            t={t}
          />
          {graphData.graph.edges.length === 0 ? <State id="collaboration-empty" title={t('insight.collaboration.empty')} body={t('insight.collaboration.emptyBody')} /> : null}
          {graphData.truncated || graphData.next_cursor ? (
            <div role="status" className="flex flex-wrap items-center justify-between gap-2 rounded border border-warning/40 bg-warning/10 p-3 text-sm" data-testid="collaboration-truncated">
              ...
              {graphData.next_cursor ? <button type="button" className="rounded border border-border bg-bg-primary px-3 py-1.5 text-sm" onClick={() => setLoadedCursor(graphData.next_cursor ?? '')}>{t('insight.collaboration.loadMore')}</button> : null}
            </div>
          ) : null}
          <SecondaryInsightPanels summary={graphData.summary} effects={graphData.effects} onSelect={setSelectedEffectId} t={t} />
        </>
      ) : null}
  ...
  return (
    <form aria-label={t('insight.collaboration.filters.label')} className="grid gap-3 rounded border border-border bg-bg-surface p-3 md:grid-cols-4" onSubmit={(event) => event.preventDefault()}>
      {[
        ...
        ['plan_id', t('insight.collaboration.filters.plan'), 'text'],
  ...
  return (
    <section className="min-h-[32rem] rounded border border-border bg-bg-surface p-3" aria-label={t('insight.collaboration.graph')} data-testid="collaboration-graph">
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-3" aria-label={t('insight.collaboration.edgeList')}>
        {shownEdges.slice(0, 48).map((edge) => <button key={edge.id} type="button" aria-pressed={selectedEffectId === edge.effect_id} onClick={() => { if (edge.effect_id) onSelectEffect(edge.effect_id); }} className="rounded border border-border px-3 py-2 text-left text-sm hover:bg-bg-subtle focus:ring-2 focus:ring-brand"><strong>{labelFor(t, edge.relation_type)}</strong> / {labelFor(t, edge.polarity)} / {t('insight.collaboration.magnitude', { value: edge.magnitude })} / x{edge.interaction_count || 0}</button>)}
      </div>
    </section>
  );
}
...
function prepareGraph(nodes: CollaborationNode[], effects: CollaborationEffect[]): { nodes: LayoutNode[]; clusters: { id: string; label: string; x: number; y: number; w: number; h: number }[] } {
  ...
  const planNodes = Array.from(enriched.values()).filter((node) => node.kind === 'plan');
  const clusterIds = planNodes.length ? planNodes.map((node) => node.plan_id ?? node.id.replace(/^plan:/, '')) : Array.from(new Set(effects.map((effect) => effect.project_id || 'organization')));
  const clusterById = new Map(clusterIds.map((id, i) => [id, { id, label: planNodes.find((node) => (node.plan_id ?? node.id.replace(/^plan:/, '')) === id)?.label ?? id, x: 46 + i * 380, y: 52, w: 330, h: 510 }]));
  ...
  Array.from(enriched.values()).filter((node) => node.kind === 'plan').forEach((node, i) => {
    const cluster = clusterById.get(node.plan_id ?? node.id.replace(/^plan:/, '')) ?? { id: node.id, label: node.label, x: 46 + i * 380, y: 52, w: 330, h: 510 };
    layout.push({ ...node, x: cluster.x + cluster.w / 2, y: cluster.y + 46, cluster: cluster.id });
  });
  Array.from(enriched.values()).filter((node) => node.kind === 'stage').forEach((node, i) => {
    const cluster = clusterById.get(node.plan_id ?? 'organization') ?? { id: 'organization', label: 'organization', x: 46, y: 52, w: 330, h: 510 };
    layout.push({ ...node, x: cluster.x + 86 + (i % 2) * 158, y: cluster.y + 112 + Math.floor(i / 2) * 82, cluster: cluster.id });
  });
  Array.from(enriched.values()).filter((node) => node.kind === 'task').forEach((node, i) => {
    const cluster = clusterById.get(node.plan_id ?? clusterIds[i % Math.max(1, clusterIds.length)] ?? 'organization') ?? { id: 'organization', label: 'organization', x: 46, y: 52, w: 330, h: 510 };
    const offset = i % 12;
    layout.push({ ...node, x: cluster.x + 78 + (offset % 2) * 160, y: cluster.y + 186 + Math.floor(offset / 2) * 52, cluster: cluster.id });
  });
...
function GraphEdgeView({ edge, a, b, selected, dimmed, onSelect, t }: { edge: CollaborationEdge; a?: LayoutNode; b?: LayoutNode; selected: boolean; dimmed: boolean; onSelect: (id: string) => void; t: Translator }) {
  ...
  const structural = edge.relation_type === 'plan_task' || edge.relation_type === 'plan_stage' || edge.relation_type === 'stage_task';
  const dash = edge.relation_type === 'assign' || edge.relation_type === 'reassign' ? undefined : structural || edge.polarity === 'neutral' ? '2 6' : '8 5';
  return (
    <g role="button" tabIndex={0} aria-label={`${labelFor(t, edge.relation_type)} ${labelFor(t, edge.polarity)} ${edge.interaction_count || 0}`} onClick={() => { if (edge.effect_id) onSelect(edge.effect_id); }} onKeyDown={(event) => { if ((event.key === 'Enter' || event.key === ' ') && edge.effect_id) onSelect(edge.effect_id); }} className={dimmed ? 'opacity-25' : undefined}>
      ...
      <text x={(a.x + b.x) / 2} y={(a.y + b.y) / 2 - 7} textAnchor="middle" className="fill-text-primary text-[11px]">{labelFor(t, edge.relation_type)} / {polarityToken(edge.polarity)} / x{edge.interaction_count || 0}</text>
    </g>
  );
}
...
function GraphNodeView({ node, focused, dimmed, onFocus, t }: { node: LayoutNode; focused: boolean; dimmed: boolean; onFocus: (id: string) => void; t: Translator }) {
  ...
  return (
    <g data-node-id={node.id} tabIndex={0} role="button" aria-label={`${kindLabel(t, node.kind)} ${node.label}`} onClick={() => onFocus(node.id)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onFocus(node.id); }} className={cls}>
      ...
      {node.kind === 'stage' ? <rect x={node.x - 52} y={node.y - 20} width="104" height="40" rx="0" className="fill-bg-subtle stroke-warning" strokeWidth={focused ? 4 : 2} data-testid="collaboration-stage-node" /> : null}
...
function Legend({ t }: { t: Translator }) {
  return (
    <div className="flex flex-wrap gap-3 text-xs text-text-muted" data-testid="collaboration-legend">
      ...
      <span><span className="mr-1 inline-block h-3 w-5 border-2 border-warning align-middle" />{t('insight.collaboration.legend.stage')}</span>
...
function mergeGraphPages(prev: CollaborationGraphResponse | null, next: CollaborationGraphResponse): CollaborationGraphResponse {
  if (!prev || prev.graph_version !== next.graph_version) return next;
  const nodes = new Map(prev.graph.nodes.map((node) => [node.id, node]));
  for (const node of next.graph.nodes) nodes.set(node.id, node);
  const edges = new Map(prev.graph.edges.map((edge) => [edge.semantic_key || edge.id, edge]));
  for (const edge of next.graph.edges) edges.set(edge.semantic_key || edge.id, edge);
  const effects = new Map(prev.effects.map((effect) => [effect.effect_id, effect]));
  for (const effect of next.effects) effects.set(effect.effect_id, effect);
  return {
    ...next,
    graph: { nodes: Array.from(nodes.values()), edges: Array.from(edges.values()) },
    effects: Array.from(effects.values()),
    summary: {
      positive_count: Array.from(effects.values()).filter((item) => item.polarity === 'positive').length,
      negative_count: Array.from(effects.values()).filter((item) => item.polarity === 'negative').length,
      neutral_count: Array.from(effects.values()).filter((item) => item.polarity === 'neutral').length,
      mixed_count: Array.from(effects.values()).filter((item) => item.polarity === 'mixed').length,
      affected_task_count: new Set(Array.from(effects.values()).map((item) => item.target_task_id)).size,
    },
  };
}
