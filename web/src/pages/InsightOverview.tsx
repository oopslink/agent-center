...
function FreshnessNotice({ data }: { data: Pick<InsightOverviewDTO, 'freshness' | 'refreshed_at'> }): React.ReactElement | null {
  ...
  return <InsightStatePanel testId={`insight-${data.freshness.state}`} tone={tone} title={t(`insight.state.${data.freshness.state}`)} body={t('insight.state.staleBody', { time: formatInsightClock(data.refreshed_at) })} />;
}
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.execution_id} className="border-t border-border-base" data-testid="insight-execution-row">
              ...
              <td className="px-3 py-2 align-top tabular-nums">{formatInsightTimeOrLabel(row.queued_at, EMPTY)}</td>
              <td className="px-3 py-2 align-top tabular-nums">{formatInsightTimeOrLabel(row.started_at, t('insight.execution.notStarted'))}</td>
              <td className="px-3 py-2 align-top tabular-nums">{formatInsightTimeOrLabel(row.finished_at, t('insight.execution.notFinished'))}</td>
              <td className="px-3 py-2 align-top tabular-nums">{formatInsightDuration(row.queue_wait_ms, t)}</td>
              <td className="px-3 py-2 align-top tabular-nums">{formatInsightDuration(row.duration_ms, t)}</td>
              <td className="px-3 py-2 align-top"><InsightQualityBadge quality={row.quality} /></td>
            </tr>
          ))}
        </tbody>
      </table>
