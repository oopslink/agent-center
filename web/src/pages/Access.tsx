    ...
    <div className="fixed inset-0 z-50" data-testid="access-role-drawer-backdrop">
      ...
      >
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-5">
          ...
          {hasHighRisk && (
            <div className="flex items-center gap-3 rounded border border-warning/30 bg-warning/10 p-3 text-xs text-text-secondary">
              <button
                type="button"
                role="switch"
                aria-checked={ack}
                aria-labelledby="access-role-risk-ack-label"
                className={`relative h-5 w-9 shrink-0 rounded-full transition-colors ${ack ? 'bg-warning' : 'bg-bg-subtle'}`}
                data-testid="access-role-risk-ack"
                onClick={() => setAck((value) => !value)}
              >
                <span aria-hidden="true" className={`absolute top-0.5 h-4 w-4 rounded-full bg-white transition-transform ${ack ? 'translate-x-4' : 'translate-x-0.5'}`} />
              </button>
