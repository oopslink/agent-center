...
describe('PlanDetail — v2.30.1 PlanDag has_graph loading→true transition (React #300 guard)', () => {
  ...
  // T981 (plan-stage-model §7): even has_graph:false plans render their stages
  // inside the DAG canvas, matching the mockup rather than a detached summary list.
  it('legacy DAG groups tasks into stage boxes with status/progress/retry', async () => {
    ...
    expect(screen.getByTestId('plan-stage-status-st-a')).toHaveTextContent('done');
    ...
    const contractToggle = screen.getByTestId('plan-stage-gate-contract-st-b-toggle');
    expect(contractToggle).toHaveAttribute('aria-expanded', 'false');
    ...
    const diagnosticsToggle = screen.getByTestId('plan-stage-gate-diagnostics-st-b-toggle');
    ...
    expect(screen.getByTestId('plan-stage-mobile-gate-contract-st-b-toggle')).toHaveAttribute('aria-expanded', 'false');
    ...
    expect(screen.getByTestId('plan-stage-mobile-gate-evidence-text-st-b-toggle')).toHaveAttribute('aria-expanded', 'false');
    expect(screen.getByTestId('plan-stage-mobile-gate-diagnostics-st-b')).toHaveTextContent('missing_browser_evidence');
    expect(screen.getByTestId('plan-stage-mobile-gate-diagnostics-st-b-toggle')).toHaveAttribute('aria-expanded', 'false');
    expect(screen.getByTestId('plan-stage-mobile-gate-contract-st-a')).toHaveTextContent('Missing acceptance contract');
    expect(screen.queryByTestId('plan-stage-mobile-gate-contract-st-a-toggle')).not.toBeInTheDocument();
    ...
    expect(screen.queryByTestId('plan-stage-mobile-gate-evidence-text-st-a-toggle')).not.toBeInTheDocument();
