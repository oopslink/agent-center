...
export default function AgentDetail(): React.ReactElement {
  ...
  if (agent.isLoading) {
    return (
      <section className="text-sm text-slate-500" data-testid="page-AgentDetail">
        Loading agent…
      </section>
    );
  }
  ...
  return (
    <section className="space-y-4" data-testid="page-AgentDetail" data-agent-name={a.name}>
      ...
      <dl className="grid grid-cols-2 gap-x-4 gap-y-2 rounded border border-slate-200 bg-white p-4 text-sm">
        <dt className="text-slate-500">CLI</dt>
        <dd className="font-mono">{a.agent_cli}</dd>
        <dt className="text-slate-500">State</dt>
        <dd>
          <span className="rounded bg-slate-100 px-2 py-0.5 text-xs uppercase">{a.state}</span>
        </dd>
        <dt className="text-slate-500">Worker</dt>
        <dd className="font-mono text-xs">{a.worker_id || '—'}</dd>
        <dt className="text-slate-500">Max concurrent</dt>
        <dd className="font-mono text-xs">{a.max_concurrent ?? '—'}</dd>
        <dt className="text-slate-500">Builtin</dt>
        <dd className="font-mono text-xs">{a.is_builtin ? 'yes' : 'no'}</dd>
      </dl>

      <section className="rounded border border-slate-200 bg-white p-4">
        <h3 className="mb-2 text-sm font-semibold text-slate-700">Active executions</h3>
        {fleet.isLoading && (
          <p className="text-xs text-slate-500" data-testid="agent-exec-loading">
            Loading fleet…
          </p>
        )}
        {fleet.isError && (
        ...
        {fleet.isSuccess && ownExecs.length === 0 && (
          <p className="text-xs text-slate-500" data-testid="agent-exec-empty">
