...
export function InstantiateModal({
  ...
  return (
    <ModalShell
      ...
      subtitle={
        <>
          把 <b>{template.name}</b> 实例化成一支活 team。<b>与 project 无关</b> —— 只建 team + 按配比建身份 + seed memory +
          绑 workflow；project 之后在详情「关联项目」里加。
        </>
      }
      footer={
        <>
    ...
    >
      ...
      <div className="mb-3 mt-1 flex items-center justify-between gap-3">
        ...
        <span className="text-[0.6875rem] text-text-muted" data-testid="instantiate-total">
          Σ {total} slots → 将建 {total} 个 agent
        </span>
      </div>
      ...
      <p className="mt-2 text-[0.6875rem] text-text-muted">
        ＊ 项目相关的事（seed 项目事实进 team repo、实际跑 plan）延后到 <span className="font-mono">associate_project</span> 时；workflow
        绑定不依赖 project。
      </p>
    </ModalShell>
  );
