  ...
  return (
    <div data-testid={`${idPrefix}-rolebuilder`}>
      {roles.map((r, i) => {
        ...
        const selectedRamRoles = r.ram_roles ?? [];
        const profiles = accessProfiles.data?.profiles ?? [];
        const selectedProfiles = profiles.filter((profile) => selectedRamRoles.includes(profileRef(profile)));
        const ramRoleOptions: EntityOption[] = profiles.map((profile) => ({
          value: profileRef(profile),
        ...
        const setRamRoles = (nextRefs: string[]) => {
          const nextProfiles = profiles.filter((profile) => nextRefs.includes(profileRef(profile)));
          patch(i, {
            ram_roles: nextRefs,
...
function profileRef(profile: { id: string; version: number }): string {
