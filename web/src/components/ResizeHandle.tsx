...
// area + a centered hairline. Shared by the ThreadSidebar and the col④ context
// panel (AppLayout) so the affordance (col-resize cursor, hover/active highlight,
// keyboard separator) is identical. Solid tokens only (border-strong / accent) —
// alpha tints on the hex CSS-var tokens render transparent (see RoleBadge note in
// ParticipantsPanel).
interface Props {
  ...
  ariaLabel: string;
  testId?: string;
  /** extra classes — e.g. `hidden md:block` to scope the grip to desktop. */
  className?: string;
}
