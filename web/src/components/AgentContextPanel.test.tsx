// v2.10.0 [T7] Members — col④ context panel: the selected agent's current task
...
import { AgentContextPanel, pickCurrentTask } from './AgentContextPanel';
import type { AgentTask } from '@/api/types';

function wi(over: Partial<AgentTask>): AgentTask {
  return {
...
describe('pickCurrentTask', () => {
  it('prefers an active item over everything else', () => {
    ...
    expect(pickCurrentTask(items)?.id).toBe('b');
  });
  it('falls back to the most-recent NON-terminal item', () => {
    ...
    expect(pickCurrentTask(items)?.id).toBe('new');
  });
  it('falls back to the most-recent item when all are terminal', () => {
    ...
    expect(pickCurrentTask(items)?.id).toBe('d2');
  });
  it('returns undefined for no items', () => {
    expect(pickCurrentTask([])).toBeUndefined();
  });
});
...
describe('AgentContextPanel', () => {
  ...
  it('shows the active work item (title, status, task handle) linking to its task', async () => {
    renderPanel();
    await waitFor(() => expect(screen.getByTestId('agent-context-task')).toBeInTheDocument());
    const card = screen.getByTestId('agent-context-task');
    expect(card).toHaveAttribute('data-status', 'active');
    expect(within(card).getByTestId('agent-context-task-link')).toHaveTextContent('Run-real acceptance');
    expect(within(card).getByTestId('agent-context-task-link')).toHaveAttribute('href', '/projects/proj-1/tasks/task-xyz');
    expect(within(card).getByTestId('agent-context-task-status')).toHaveTextContent('Running');
  ...
  it('shows an empty state when the agent has no work items', async () => {
    server.use(http.get('/api/agents/:id/tasks', () => HttpResponse.json({ tasks: [] })));
    renderPanel();
    await waitFor(() => expect(screen.getByTestId('agent-context-no-task')).toBeInTheDocument());
  ...
  it('shows "Not part of a plan" when no plan contains the current task', async () => {
    server.use(
      http.get('/api/agents/:id/tasks', () =>
        HttpResponse.json({ tasks: [wi({ id: 'wi-1', status: 'active', task_id: 'task-orphan', task_title: 'Orphan', project_id: 'proj-1' })] }),
      ),
