import type { AIRuntimeCatalog, RuntimeCLI, RuntimeModel, RuntimeProfile } from '@/api/aiRuntime';
...
export function runtimeDefaultChoice(catalog: AIRuntimeCatalog | undefined): RuntimeChoice | null {
  ...
  const profile = runtimeDefaultProfile(catalog);
  if (profile?.enabled) {
    const cli = findRuntimeCLI(catalog, profile.cli_key);
    const model = findRuntimeModel(catalog, profile.model_key);
...
function runtimeDefaultProfile(catalog: AIRuntimeCatalog): RuntimeProfile | undefined {
  const id = catalog.default_runtime_profile_id;
  if (!id) return undefined;
  return catalog.profiles.find((profile) => profile.id === id || profile.key === id);
}
