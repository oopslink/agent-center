...
func TestPrintFleet_HumanIncludesAllSegments(t *testing.T) {
	...
	snap := query.FleetSnapshot{
		Executions:        []query.FleetExecutionRow{{ExecutionID: "E-1", TaskID: "T-1", WorkerID: "W-1", Status: "working"}},
	...
	for _, want := range []string{"E-1", "W-1", "IR-1", "I-1"} {
...
func TestQueryCmd_AllResources_NoCrash(t *testing.T) {
	...
	_ = app.TaskRepo.Save(context.Background(), tk)
	cmd := findCmd(app.ObservabilityCommands(), "query")
	for _, r := range []string{"tasks", "executions", "workers", "issues", "input_requests", "proposals", "events", "decisions"} {
