...
func installCenterHandler(fs *flag.FlagSet) Handler {
	...
	force := fs.Bool("force", false, "force the binary swap even when the installed version+commit match this build (v2.7.1 #234). Use to redeploy an identical build or recover a partial install.")
...
func detectExistingInstall(prefix, thisVersion string) (InstallState, string, error) {
	...
	// v2.7.1 #234: the version strings match — disambiguate by build commit.
	// A same-version-different-commit rebuild (the #217 dogfood bug: an
	// in-place upgrade silently skipped the binary swap because version-string
	// comparison saw no change) must be classified as an upgrade so the new
	// binary actually swaps in. Back-compat: a legacy install with no COMMIT
	// marker falls back to version-only (SameVersion), preserving the pre-#234
	// idempotency for installs predating this file.
	commitFile := filepath.Join(currentLink, "COMMIT")
	cdata, cerr := os.ReadFile(commitFile)
	if cerr != nil {
		if os.IsNotExist(cerr) {
			return InstallStateSameVersion, currentVersion, nil
		}
		return InstallStateUnknown, "", fmt.Errorf("read %s: %w", commitFile, cerr)
	}
	if strings.TrimSpace(string(cdata)) == installerCommit() {
		return InstallStateSameVersion, currentVersion, nil
	}
	return InstallStateUpgrade, currentVersion, nil
}

// applyForceState upgrades a SameVersion classification to Upgrade when
// --force is set (v2.7.1 #234), so the operator can re-swap an identical
// build (same version + commit) — e.g. to redeploy after a manual edit or
// recover a half-applied install. Fresh / Upgrade / Unknown are unchanged
// (force only overrides the idempotent no-op).
func applyForceState(state InstallState, force bool) InstallState {
	if force && state == InstallStateSameVersion {
		return InstallStateUpgrade
	}
	return state
}
...
// installerCommit returns the binary's own build commit (the git short
// sha injected at link time via -X main.buildCommit). Falls back to
// "unknown" for `go run` / unversioned builds. v2.7.1 #234: lets the
// installer disambiguate a same-version rebuild (different sha) from a
// true no-op so an in-place upgrade actually swaps the binary.
func installerCommit() string {
	if c := installBuildCommit(); c != "" {
		return c
	}
	return "unknown"
}

// installBuildCommit mirrors installBuildVersion: a test seam over the
// linker-injected main.buildCommit. Test builds return "" → installerCommit
// reports "unknown".
var installBuildCommit = func() string { return "" }

// SetInstallBuildCommit threads main.buildCommit into the install command
// (v2.7.1 #234). No-ops for the "unknown" sentinel so `go run` stays
// commit-agnostic. Tests mutate installBuildCommit directly with a
// restore-on-defer pattern.
func SetInstallBuildCommit(c string) {
	...
	bc := c
	installBuildCommit = func() string { return bc }
}
