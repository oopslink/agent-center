...
func (r *LocalRuntime) materializeTaskInputPackage(ctx context.Context, agentID, taskID, execID, workspaceDir string, task *centerTaskDetail) (*executor.TaskInputRef, error) {
	...
	for _, f := range files {
		...
		if strings.TrimSpace(f.Filename) == "" {
			return nil, fmt.Errorf("task-input materialization: task %s file %s has empty filename in authoritative list_files metadata", taskID, f.URI)
		}
		if strings.TrimSpace(f.MimeType) == "" {
			return nil, fmt.Errorf("task-input materialization: task %s file %s has empty mime_type in authoritative list_files metadata", taskID, f.URI)
		}
		...
		wantSHA := strings.ToLower(strings.TrimSpace(f.SHA256))
		if !isSHA256Hex(wantSHA) {
			return nil, fmt.Errorf("task-input materialization: task %s file %s has missing/invalid sha256 in authoritative list_files metadata", taskID, f.URI)
		}
		...
		if gotSHA != wantSHA {
			return nil, fmt.Errorf("task-input materialization: sha256 mismatch for %s: listed=%s downloaded=%s", f.URI, wantSHA, gotSHA)
		}
