...
func codexComputerUseAvailable(sourceCodexHome string) bool {
	...
	paths := codexNodeRuntimePaths(sourceCodexHome)
	if !regularFileExists(paths.nodeRepl) || !regularFileExists(paths.node) || !codexDirExists(paths.nodeModuleDirs) {
		return false
	}
	return strings.TrimSpace(codexComputerUseServicePath(sourceCodexHome)) != ""
}

type codexNodeRuntime struct {
	nodeRepl       string
	node           string
	nodeModuleDirs string
}

func codexNodeRuntimePaths(sourceCodexHome string) codexNodeRuntime {
	src := strings.TrimSpace(sourceCodexHome)
	if src != "" {
		root := filepath.Join(src, "cua_node")
		paths := codexNodeRuntime{
			nodeRepl:       filepath.Join(root, "bin", "node_repl"),
			node:           filepath.Join(root, "bin", "node"),
			nodeModuleDirs: filepath.Join(root, "lib", "node_modules"),
		}
		if regularFileExists(paths.nodeRepl) || regularFileExists(paths.node) || codexDirExists(paths.nodeModuleDirs) {
			return paths
		}
	}
	return codexNodeRuntime{
		nodeRepl:       codexNodeReplCommand,
		node:           codexNodeCommand,
		nodeModuleDirs: codexNodeModuleDirs,
	}
}
