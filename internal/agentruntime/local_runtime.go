...
type LocalRuntime struct {
	...
	// the control_loaded recovery hook. Reload/restart advances generation and must run
	// another check; duplicate control_loaded reports in one generation must not.
...
func (r *LocalRuntime) reportControlLoaded(agentID string, spec StartSpec, info controlLoadedInfo) {
	defer r.triggerControlLoadedRecovery(info.Generation)
...
// control_loaded boundary. It is once per generation, so duplicate reports are
// harmless while every reload/restart generation checks again.
