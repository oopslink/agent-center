...
func TestInsightsAPI_UnavailableReturnsFreshnessEnvelope(t *testing.T) {
	deps, db := setupAPIWithAuth(t)
	sess := setupTestSession(t, db, deps)
	...
	if out.Error != "insight_unavailable" || out.Freshness.State != "unavailable" {
		t.Fatalf("503 body = %+v, want stable error with unavailable freshness", out)
	}
}
