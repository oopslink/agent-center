...
func (s *Server) importTemplateHandler(w http.ResponseWriter, r *http.Request) {
	...
	for _, rr := range req.Roles {
		...
		slots = append(slots, team.RoleSlot{
			Config: team.RoleConfig{Role: role, CLI: cli, Model: model, CapabilityTags: tags, AccessRequirements: rr.AccessRequirements, RAMRoleKeys: rr.RAMRoleKeys, MaxConcurrency: maxConc},
			Count:  count,
		})
	}
	var valid bool
