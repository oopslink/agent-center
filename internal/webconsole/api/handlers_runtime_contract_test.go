...
func wireRuntimeCatalogForTest(t *testing.T, db *sql.DB, deps *HandlerDeps, orgID string) {
	...
	profile, rev, err := deps.RuntimeCatalog.CreateProfile(ctx, orgID, actor, rev, airuntime.RuntimeProfile{
		Key: "default-coding", Name: "Default coding", CLIKey: "claude-code", ModelKey: "claude-opus-4-8",
		Parameters: map[string]any{}, Enabled: true,
	})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := deps.RuntimeCatalog.SetDefaultProfile(ctx, orgID, actor, profile.ID, rev); err != nil {
		t.Fatal(err)
	}
}
