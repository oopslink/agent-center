...
func TestAIRuntimeCRUDRevisionConflictHTTP(t *testing.T) {
	deps, db := setupAPIWithAuth(t)
	n := 0
	deps.RuntimeCatalog = airuntime.NewService(airuntimesql.NewRepository(db), func() string {
		n++
		return fmt.Sprintf("runtime-conflict-%d", n)
	})
	owner := setupTestSession(t, db, deps)
	server := newTestServer(t, deps)
	defer server.Close()
	...
	stale := `{"expected_revision":0,"value":{"key":"gpt-5","model_key":"gpt-5","display_name":"stale","compatible_cli_keys":["codex"],"default_parameters":{},"enabled":true}}`
	req, _ := http.NewRequest(http.MethodPatch, orgScopedURL(server.URL+"/api/ai-runtime/models/"+created.Entry.ID, owner.OrgSlug), strings.NewReader(stale))
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(owner.Cookie)
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
	...
	var body airuntime.Error
	if err := json.NewDecoder(resp.Body).Decode(&body); err != nil {
		t.Fatal(err)
	}
	if body.Reason != airuntime.ReasonRevisionConflict {
		t.Fatalf("reason=%q want %q", body.Reason, airuntime.ReasonRevisionConflict)
	}
}

func TestAIRuntimeModelOnlyImportPreservesProfilesAndCLIsHTTP(t *testing.T) {
	deps, db := setupAPIWithAuth(t)
	n := 0
	deps.RuntimeCatalog = airuntime.NewServiceWithValidationKey(
		airuntimesql.NewRepository(db),
		func() string {
			n++
			return fmt.Sprintf("runtime-model-import-%d", n)
		},
		[]byte("0123456789abcdef0123456789abcdef"),
	)
	owner := setupTestSession(t, db, deps)
	...
	resp.Body.Close()
	setDefaultURL := orgScopedURL(server.URL+"/api/ai-runtime/default-profile", owner.OrgSlug)
	req, _ := http.NewRequest(http.MethodPut, setDefaultURL, strings.NewReader(fmt.Sprintf(`{"expected_revision":%d,"profile_id":"%s"}`, profileResult.Revision, profileResult.Entry.ID)))
	req.Header.Set("Content-Type", "application/json")
	req.AddCookie(owner.Cookie)
	resp, err := http.DefaultClient.Do(req)
	if err != nil {
	...
	before, err := deps.RuntimeCatalog.Catalog(context.Background(), owner.OrgID)
	if err != nil {
		t.Fatal(err)
	}
	doc, err := deps.RuntimeCatalog.Export(context.Background(), owner.OrgID)
	if err != nil {
		t.Fatal(err)
	}
	doc.Runtime.Models = append(doc.Runtime.Models, airuntime.ExportModel{
		...
		Enabled: true, ContextWindow: 128000, InputCost: 0.15, OutputCost: 0.6, Tier: "small",
	})
	previewPayload, _ := json.Marshal(airuntime.PreviewRequest{Strategy: airuntime.StrategyMerge, Document: doc})
	resp = orgScopedPost(t, server.URL+"/api/ai-runtime/import/preview", string(previewPayload), owner)
	if resp.StatusCode != http.StatusOK {
		var body any
		_ = json.NewDecoder(resp.Body).Decode(&body)
		t.Fatalf("preview status=%d body=%+v", resp.StatusCode, body)
	...
	after, err := deps.RuntimeCatalog.Catalog(context.Background(), owner.OrgID)
	if err != nil {
		t.Fatal(err)
	}
	if len(after.CLIs) != len(before.CLIs) || len(after.Profiles) != len(before.Profiles) || after.DefaultProfileID != before.DefaultProfileID {
		t.Fatalf("import changed non-model catalog data: before=%+v after=%+v", before, after)
	}
	found := false
	for _, model := range after.Models {
		if model.Key == "gpt-5-mini" && model.ModelKey == "gpt-5-mini" {
			found = true
		}
	}
	if !found {
		t.Fatalf("imported model missing: %+v", after.Models)
	}
}
