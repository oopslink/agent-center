...
func TestT1343LegacyAccessApplyMustMatchPermissionsEffectiveState(t *testing.T) {
	deps, db := setupAPIWithAuth(t)
	sess := setupTestSession(t, db, deps)
	deps.Authorizer = authz.New(authz.Deps{DB: db})
	seedT1343JoinedHuman(t, db, sess.OrgID, "grantuser", "Grant User")
...
func TestT1343LegacyAccessOverviewMustIncludeCustomRolePermissions(t *testing.T) {
	deps, db := setupAPIWithAuth(t)
	sess := setupTestSession(t, db, deps)
	svc := authz.New(authz.Deps{DB: db})
	deps.Authorizer = svc
	seedT1343JoinedHuman(t, db, sess.OrgID, "customuser", "Custom User")
	server := newTestServer(t, deps)
	defer server.Close()

	if _, err := svc.ApplyBatch(context.Background(), authz.BatchRequest{
		IdempotencyKey: "t1343-web-custom-role",
		ActorRef:       authz.UserSubject(sess.IdentityID),
		OrgID:          sess.OrgID,
		Operations: []authz.BatchOperation{
			{Type: "upsert_role", Role: authz.RoleInput{ID: "role-t1343-settings", Name: "settings-manager"}},
			{Type: "set_role_permissions", Role: authz.RoleInput{ID: "role-t1343-settings"}, Permissions: []authz.RolePermissionInput{{PermissionKey: "org.settings.manage", ResourceKind: "org"}}},
			{Type: "assign_role", Assignment: authz.AssignmentInput{ID: "asgn-t1343-settings", SubjectRef: "user:customuser", RoleID: "role-t1343-settings", Resource: authz.ResourceScope{Kind: "org", ID: sess.OrgID}}},
		},
	}); err != nil {
...
func seedT1343JoinedHuman(t *testing.T, db *sql.DB, orgID, identityID, name string) {
	t.Helper()
