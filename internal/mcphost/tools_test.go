...
func TestJSONToolsForwarding(t *testing.T) {
	cases := []struct {
		...
		{
			toolName: "reset_task",
			args:     map[string]any{"task_id": "t-1", "confirmed_dead": true},
			wantTool: "reset_task",
			wantBody: map[string]any{"agent_id": "agent-X", "task_id": "t-1", "confirmed_dead": true},
		},
		{
			toolName: "fail_task",
			args:     map[string]any{"task_id": "t-1", "reason": "stuck"},
			wantTool: "fail_task",
			wantBody: map[string]any{"agent_id": "agent-X", "task_id": "t-1", "reason": "stuck"},
		},
		{
			toolName: "complete_task",
			args:     map[string]any{"task_id": "t-1", "summary": "done"},
			wantTool: "complete_task",
			wantBody: map[string]any{"agent_id": "agent-X", "task_id": "t-1", "summary": "done", "executor_id": "", "head_sha": ""},
		},
		{
...
func TestRequireTools_TieredCatalogIncludesSupervisorCore(t *testing.T) {
	if err := RequireTools(context.Background(), Config{AgentID: "agent-x", TierTools: true}, "post_message", "list_my_tasks", "search_tools", "runtime_deploy_restart", "runtime_deploy_status"); err != nil {
