...
func makeRuntimeDeployRestart(cfg Config) mcp.ToolHandlerFor[runtimeDeployRestartArgs, any] {
	return func(ctx context.Context, _ *mcp.CallToolRequest, args runtimeDeployRestartArgs) (*mcp.CallToolResult, any, error) {
		body := map[string]any{
			"agent_id":   cfg.AgentID,
...
//   - start_plan / pause_plan / resume_plan / reopen_plan / complete_plan / discard_plan : {plan_id}
