...
// TestMigration0058_BuiltinPlanBackfill proves the ADR-0047 data migration
// (0058_v291_builtin_plan) backfills correctly:
...
//
// Strategy: full Up lands at 56 (pm_plans/pm_tasks/plan_id all exist, builtin
// column not yet). We seed one project + four tasks (assigned-non-terminal,
// unassigned, completed-assigned, already-in-another-plan) then Up to 58 which
// applies 0058's schema + backfill, and assert each task's plan_id.
func TestMigration0058_BuiltinPlanBackfill(t *testing.T) {
	...
	// Apply 0057 + 0058.
	if err := mig.Up(ctx); err != nil {
		t.Fatalf("re-Up (apply 0057+0058): %v", err)
	}
