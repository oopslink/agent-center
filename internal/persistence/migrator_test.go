...
func TestMigration0143FailsClosedWhenRoleAccessReferencedByTeamRole(t *testing.T) {
	...
	err = m.Up(ctx)
	if err == nil || !strings.Contains(err.Error(), "migration 0143 blocked") || !strings.Contains(err.Error(), "role=role-access-blocked") || !strings.Contains(err.Error(), "team_role_refs=\"team-t1499:reviewer\"") {
...
func TestMigration0143FailsClosedForNonSinglePermissionRoleAccess(t *testing.T) {
	db, err := Open(t.TempDir() + "/test.db")
	if err != nil {
		t.Fatal(err)
	}
	...
	now := time.Date(2026, 8, 24, 12, 0, 0, 0, time.UTC).Format(time.RFC3339Nano)
	for _, stmt := range []string{
		`INSERT INTO authorization_roles (id, org_id, kind, stable_key, scope_kind, name, description, created_by, created_at, updated_at, version) VALUES ('role-access-multiperm', 'org-t1499', 'custom', 'role-access-multiperm', 'org', 'ambiguous role access', '', 'system', '` + now + `', '` + now + `', 1)`,
...
func TestMigration0143DirectCarrierEquivalenceIdempotentAndRollback(t *testing.T) {
	db, err := Open(t.TempDir() + "/test.db")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, stmt := range []string{
		...
		`INSERT INTO authorization_roles (id, org_id, kind, stable_key, scope_kind, name, description, created_by, created_at, updated_at, version) VALUES ('role-access-single', 'org-mig', 'custom', 'role-access-single', 'org', 'legacy direct carrier', '', 'system', '` + now + `', '` + now + `', 1)`,
	...
	before := snapshot0143Carrier(t, db)
	if before.Kind != "custom" || before.Managed != "" || before.Visibility != "" || before.LiveAssignments != 3 || before.RevokedAssignments != 1 || before.EffectiveAllows != 0 || before.EligibleDirectGrants != 1 || before.ExpiredActive != 1 || before.WrongScopeActive != 1 || !before.DenyPrecedence {
		t.Fatalf("before snapshot=%+v", before)
	}
	if err := m.Up(ctx); err != nil {
		t.Fatalf("Up 0143: %v", err)
	}
	after := snapshot0143Carrier(t, db)
	if after.Kind != "managed" || after.Managed != "1" || after.Visibility != "internal" || after.LiveAssignments != before.LiveAssignments || after.RevokedAssignments != before.RevokedAssignments || after.PermissionCount != before.PermissionCount || after.ExpiresAt != before.ExpiresAt || after.EffectiveAllows != before.EffectiveAllows || after.EligibleDirectGrants != before.EligibleDirectGrants || after.ExpiredActive != before.ExpiredActive || after.WrongScopeActive != before.WrongScopeActive || after.DenyPrecedence != before.DenyPrecedence {
		t.Fatalf("after snapshot=%+v before=%+v", after, before)
	}
	if err := m.Up(ctx); err != nil {
		t.Fatalf("second Up: %v", err)
	}
	replayed := snapshot0143Carrier(t, db)
	...
	rolledBack := snapshot0143Carrier(t, db)
	if rolledBack.Kind != "custom" || rolledBack.LiveAssignments != before.LiveAssignments || rolledBack.RevokedAssignments != before.RevokedAssignments || rolledBack.PermissionCount != before.PermissionCount || rolledBack.ExpiresAt != before.ExpiresAt || rolledBack.EffectiveAllows != before.EffectiveAllows || rolledBack.EligibleDirectGrants != before.EligibleDirectGrants || rolledBack.ExpiredActive != before.ExpiredActive || rolledBack.WrongScopeActive != before.WrongScopeActive || rolledBack.DenyPrecedence != before.DenyPrecedence {
		t.Fatalf("rollback snapshot=%+v before=%+v", rolledBack, before)
	}
}

type migration0143Snapshot struct {
	Kind                 string
	Managed              string
	Visibility           string
	Version              int
...
func snapshot0143Carrier(t *testing.T, db *sql.DB) migration0143Snapshot {
	t.Helper()
	var out migration0143Snapshot
	rows, err := db.Query(`SELECT name FROM pragma_table_info('authorization_roles') WHERE name IN ('managed', 'visibility')`)
	if err != nil {
		t.Fatal(err)
	}
	hasManaged := false
	...
	rows.Close()
	query := `SELECT kind, version`
	if hasManaged {
		query += `, CAST(managed AS TEXT)`
	} else {
		query += `, ''`
	}
	if hasVisibility {
	...
	query += ` FROM authorization_roles WHERE id = 'role-access-single'`
	if err := db.QueryRow(query).Scan(&out.Kind, &out.Version, &out.Managed, &out.Visibility); err != nil {
		t.Fatal(err)
	}
	...
	if !out.DenyPrecedence {
		out.EffectiveAllows = out.EligibleDirectGrants
	}
	return out
}
