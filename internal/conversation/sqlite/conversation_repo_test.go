...
func TestConversationRepo_SaveAndFindByID(t *testing.T) {
	r := setupDB(t)
	c := mkConv(t, "conv-1", conversation.ConversationKindProjectChannel, "general")
	...
	if got.Name() != "general" || got.Kind() != conversation.ConversationKindProjectChannel {
		t.Fatalf("got %+v", got)
	}
...
func TestConversationRepo_FindByName(t *testing.T) {
	r := setupDB(t)
	if err := r.Save(context.Background(), mkConv(t, "c-1", conversation.ConversationKindProjectChannel, "general")); err != nil {
		t.Fatal(err)
	}
...
func TestConversationRepo_Save_DuplicateChannelName(t *testing.T) {
	r := setupDB(t)
	if err := r.Save(context.Background(), mkConv(t, "c-a", conversation.ConversationKindProjectChannel, "shared")); err != nil {
		t.Fatal(err)
	}
	c2 := mkConv(t, "c-b", conversation.ConversationKindProjectChannel, "shared")
...
func TestConversationRepo_UpdateParticipants(t *testing.T) {
	r := setupDB(t)
	c := mkConv(t, "c-1", conversation.ConversationKindProjectChannel, "ch")
	_ = r.Save(context.Background(), c)
...
func TestConversationRepo_FindByParent(t *testing.T) {
	r := setupDB(t)
	parent := mkConv(t, "p-1", conversation.ConversationKindProjectChannel, "p")
...
func TestConversationRepo_Find_KindAndStatus(t *testing.T) {
	r := setupDB(t)
	_ = r.Save(context.Background(), mkConv(t, "c-a", conversation.ConversationKindProjectChannel, "a"))
