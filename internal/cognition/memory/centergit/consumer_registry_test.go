...
func TestTeamRuleIndexAndBodyReadAreCommitBound(t *testing.T) {
	...
	updateRuleBodyAtHEAD(t, host, "team-index", idx.Rules[0].SourcePath, "new body")
...
func TestTeamRuleIndexBudgetsEnforcedOnWriteAndRead(t *testing.T) {
	...
	dir := t.TempDir()
	store := NewStore(dir, nil, WithIDGen(func() string { return "id" }))
...
func updateRuleBodyAtHEAD(t *testing.T, host *Host, teamID, sourcePath, body string) {
	t.Helper()
	home := t.TempDir()
