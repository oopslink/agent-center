...
func TestSelfHeal_OnTickRelaunchesAfterBackoff(t *testing.T) {
	...
	c.recordCrashAndSchedule("ag-1", 7 /*version*/, true /*hadWork*/, "wi-7" /*workItemID*/, "bad-model-x" /*model*/, "agent-center-dev4" /*displayName*/, "" /*promptDescription*/, map[string]string{"AC_TEST_PROFILE": "on"}, false, "boom")
	...
	// T469: the display_name captured at crash must survive the managedAgent delete and
	// reach the self-heal relaunch's session config (→ supervisor → git author NAME).
	if got := rs.last().cfg.DisplayName; got != "agent-center-dev4" {
		t.Fatalf("self-heal relaunch must carry display_name across the crash, got %q", got)
	}
	if got := rs.last().cfg.AgentEnv["AC_TEST_PROFILE"]; got != "on" {
		t.Fatalf("self-heal relaunch must carry profile env across the crash, got %q", got)
	}
...
func TestSelfHeal_IdleRelaunchFreshNoResume(t *testing.T) {
	...
	// relaunch at now+1s.
	c.recordCrashAndSchedule("ag-1", 3 /*version*/, false /*hadWork*/, "" /*workItemID*/, "" /*model*/, "" /*displayName*/, "" /*promptDescription*/, nil, false, "idle-crash")
...
func TestSelfHeal_CircuitBreaksAndClearUnlatches(t *testing.T) {
	...
	for i := 0; i < 6; i++ {
		state := c.recordCrashAndSchedule("ag-1", 1, false, "" /*workItemID*/, "" /*model*/, "" /*displayName*/, "" /*promptDescription*/, nil, false, "boom")
		want := "error"
		if i == 5 {
			want = "failed"
		}
