...
func (c *AgentController) recordCrashAndSchedule(agentID string, version int, hadWork bool, taskID, model, displayName, promptDescription, msg string) string {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *AgentController) OnTick(ctx context.Context) {
	...
	for id, e := range c.selfHeal {
		...
		dues = append(dues, due{agentID: id, version: e.version, nudge: e.nudge, taskID: e.taskID, model: e.model, displayName: e.displayName, promptDescription: e.promptDescription, attempt: e.crashCount})
	...
	for _, d := range dues {
		c.selfHealRelaunch(ctx, d.agentID, d.version, d.nudge, d.taskID, d.model, d.displayName, d.promptDescription, d.attempt)
	}
...
func (c *AgentController) selfHealRelaunch(ctx context.Context, agentID string, version int, nudge bool, taskID, model, displayName, promptDescription string, attempt int) {
	...
	if rerr := c.bootReapRelaunch(ctx, agentID, home, version, nudge, taskID, model, displayName, promptDescription); rerr != nil {
		...
		state := c.recordRelaunchFailAndSchedule(agentID, version, nudge, taskID, model, displayName, promptDescription, rerr.Error())
		if state == "failed" {
...
func (c *AgentController) recordRelaunchFailAndSchedule(agentID string, version int, nudge bool, taskID, model, displayName, promptDescription, msg string) string {
	c.mu.Lock()
	defer c.mu.Unlock()
	e := c.selfHeal[agentID]
	if e == nil {
