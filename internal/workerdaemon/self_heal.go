...
func (c *AgentController) recordCrashAndSchedule(agentID string, version int, hadWork bool, taskID, model, displayName, msg string) string {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *AgentController) OnTick(ctx context.Context) {
	...
	for id, e := range c.selfHeal {
		...
		dues = append(dues, due{agentID: id, version: e.version, nudge: e.nudge, taskID: e.taskID, model: e.model, displayName: e.displayName, attempt: e.crashCount})
	...
	for _, d := range dues {
		c.selfHealRelaunch(ctx, d.agentID, d.version, d.nudge, d.taskID, d.model, d.displayName, d.attempt)
	}
...
func (c *AgentController) selfHealRelaunch(ctx context.Context, agentID string, version int, nudge bool, taskID, model, displayName string, attempt int) {
	...
	if rerr := c.bootReapRelaunch(ctx, agentID, home, version, nudge, taskID, model, displayName); rerr != nil {
		...
		state := c.recordRelaunchFailAndSchedule(agentID, version, nudge, taskID, model, displayName, rerr.Error())
		if state == "failed" {
...
func (c *AgentController) recordRelaunchFailAndSchedule(agentID string, version int, nudge bool, taskID, model, displayName, msg string) string {
	c.mu.Lock()
	defer c.mu.Unlock()
	e := c.selfHeal[agentID]
	if e == nil {
