...
func TestBootReapRelaunch_ResumeGatedOnCompletedTurn(t *testing.T) {
	...
	t.Run("prior completed a turn → resumes (even idle)", func(t *testing.T) {
		...
		if rerr := c.bootReapRelaunch(context.Background(), "ag-r", home, 1, false /*nudge*/, "", "", "", "", nil, false); rerr != nil {
	...
	t.Run("prior never completed a turn → fresh, no resume (even with work)", func(t *testing.T) {
		...
		if rerr := c.bootReapRelaunch(context.Background(), "ag-f", home, 1, true /*nudge*/, "wi-1", "", "", "", nil, false); rerr != nil {
			t.Fatalf("bootReapRelaunch: %v", rerr)
		}
...
func TestBootReapRelaunch_CarriesPromptDescription(t *testing.T) {
	...
	if rerr := c.bootReapRelaunch(context.Background(), "ag-p", home, 1, false /*nudge*/, "", "", "", "persona text", nil, false); rerr != nil {
		t.Fatalf("bootReapRelaunch: %v", rerr)
	}
