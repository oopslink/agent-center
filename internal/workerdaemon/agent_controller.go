...
func (c *AgentController) reconcileRunning(ctx context.Context, pl reconcilePayload) error {
	...
	// is still held).
	if err := c.startSession(ctx, pl.AgentID, pl.Version, false /*forkResume*/, false /*resume*/, pl.Model, pl.DisplayName, pl.CLI, pl.PromptDescription); err != nil {
...
func (c *AgentController) startSession(ctx context.Context, agentID string, version int, forkResume, resume bool, model, displayName, cli, promptDescription string) error {
	...
	ma := &managedAgent{agentID: agentID, appliedVersion: version, model: model, displayName: displayName, promptDescription: promptDescription}
	c.mu.Lock()
	c.agents[agentID] = ma
	c.mu.Unlock()
...
func (c *AgentController) onExit(agentID string, exitErr error) {
	...
	state := c.recordCrashAndSchedule(agentID, version, hadWork, taskID, model, displayName, promptDescription, msg)
	if state != "" {
		ma.lifecycleOnce.Do(func() {
