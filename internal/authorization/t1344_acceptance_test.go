...
func TestT1344ServiceContractMixedBatchSourcesExpiryReclaimAndAudit(t *testing.T) {
	...
	req := BatchRequest{
		IdempotencyKey: "t1344-mixed-human-agent",
	...
	if _, err := svc.ApplyBatch(ctx, BatchRequest{IdempotencyKey: "t1344-agent-high-risk", ActorRef: "user:user-owner", OrgID: "org-1", Operations: []BatchOperation{{Type: "assign_role", Assignment: AssignmentInput{SubjectRef: "agent:mem-agent", RoleID: "sys-org-owner", Resource: ResourceScope{Kind: "org", ID: "org-1"}}}}}); !errors.Is(err, ErrInvalid) {
		t.Fatalf("agent high-risk assignment err=%v, want ErrInvalid", err)
	}
	if _, err := svc.ApplyBatch(ctx, BatchRequest{IdempotencyKey: "t1344-self-escalate", ActorRef: "user:user-admin", OrgID: "org-1", Operations: []BatchOperation{{Type: "assign_role", Assignment: AssignmentInput{SubjectRef: "user:user-admin", RoleID: "sys-org-owner", Resource: ResourceScope{Kind: "org", ID: "org-1"}}}}}); err == nil {
		t.Fatal("non-owner self-escalation unexpectedly succeeded")
	}

	if _, err := svc.ApplyBatch(ctx, BatchRequest{IdempotencyKey: "t1344-custom-role-impact", ActorRef: "user:user-owner", OrgID: "org-1", Operations: []BatchOperation{{Type: "set_role_permissions", Role: RoleInput{ID: "role-team-proposer"}, Permissions: []RolePermissionInput{{PermissionKey: "team.git.read", ResourceKind: "team"}}}}}); err != nil {
	...
	if _, err := svc.ApplyBatch(ctx, BatchRequest{IdempotencyKey: "t1344-expired-grant", ActorRef: "system", OrgID: "org-1", Operations: []BatchOperation{
	...
	execMany(t, db, `INSERT INTO authorization_role_assignments
		...
		VALUES ('asgn-t1344-last-owner', 'org-1', 'user:user-owner', 'sys-org-owner', 'org', 'org-1', 'system', ?)`, testNow())
	if _, err := svc.RevokeBatch(ctx, BatchRequest{IdempotencyKey: "t1344-last-owner", ActorRef: "system", OrgID: "org-1", Operations: []BatchOperation{{Revoke: RevokeInput{AssignmentID: "asgn-t1344-last-owner"}}}}); !errors.Is(err, ErrConflict) {
