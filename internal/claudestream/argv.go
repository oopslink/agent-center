...
// SessionUUID derives a DETERMINISTIC RFC 4122 v5 UUID from an agent id, for use
// as claude's --session-id. claude rejects a non-UUID session-id (validated
...
func SessionUUID(agentID string) string {
	h := sha1.New()
...
func BuildStreamingArgv(agentID, binary, mcpConfigPath string, env map[string]string) ([]string, error) {
	if agentID == "" {
		return nil, errors.New("claudestream: agent_id required")
	}
	adapter := claudecode.New(binary)
	req := agentadapter.SpawnRequest{
		ExecutionID: SessionUUID(agentID),
