...
// blocked/completed/verified/discarded/reopened do NOT count as running. This
// guards against the precondition OVER-reaching (e.g. treating TaskBlocked —
// semantically active but not executing — as blocking) or under-checking.
//
// Inverse-mutation: drop the running-task check in ArchivePlan → running_blocks
// FAILS. Broaden it to also block TaskBlocked → blocked_does_not_block FAILS.
func TestArchivePlan_RunningTaskOnly_StatusCoverage(t *testing.T) {
	...
	t.Run("blocked_does_not_block", func(t *testing.T) {
		h, planID, a := archivable(t)
		if err := h.svc.SetTaskStatus(h.ctx, a, pm.TaskRunning, "user:a"); err != nil {
			t.Fatal(err)
		}
		if err := h.svc.SetTaskStatus(h.ctx, a, pm.TaskBlocked, "user:a"); err != nil {
			t.Fatal(err)
		}
		// blocked is active-but-not-executing → must NOT trip the running-task gate.
		if err := h.svc.ArchivePlan(h.ctx, planID, "user:a"); err == pm.ErrPlanHasRunningTasks {
			t.Fatalf("blocked member task must NOT block archive (only TaskRunning blocks) — got ErrPlanHasRunningTasks")
		}
	})
}
