...
func TestS2B_RecordStageGateVerdict_FailClosedOnRemoteAndAuthority(t *testing.T) {
	...
	if _, err := h.svc.RecordStageGateVerdict(h.ctx, RecordStageGateVerdictCommand{
		GateTaskID: gate, Outcome: pm.GateVerdictPass, Evidence: "ok", ReviewedSHA: s2bOther, IdempotencyKey: "wrong-sha", Actor: "user:a",
	}); !errors.Is(err, ErrDeliveryVerificationFailed) {
		t.Fatalf("wrong reviewed sha err=%v, want ErrDeliveryVerificationFailed", err)
	}
	verifier.bySHA[s2bGood] = DeliveryVerification{CandidateExists: true, RefMatches: false, Pushed: false, BaseIsAncestor: true, RemoteSHA: s2bOther}
	if _, err := h.svc.RecordStageGateVerdict(h.ctx, RecordStageGateVerdictCommand{
		GateTaskID: gate, Outcome: pm.GateVerdictPass, Evidence: "ok", ReviewedSHA: s2bGood, IdempotencyKey: "moving-branch", Actor: "user:a",
	}); !errors.Is(err, ErrDeliveryVerificationFailed) {
		t.Fatalf("moving branch/missing push err=%v, want ErrDeliveryVerificationFailed", err)
	}
