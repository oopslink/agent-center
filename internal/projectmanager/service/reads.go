...
// True when the task is terminal (completed/discarded) or PARKED (blocked — no
// executor is in flight, so the slot is released). False for a
...
func (s *Service) planDetail(ctx context.Context, p *pm.Plan) (*PlanDetail, error) {
	...
	supersessions, err := s.plans.ListSupersessions(ctx, p.ID())
	if err != nil {
		return nil, err
	}
	...
	return &PlanDetail{Plan: p, Tasks: tasks, View: pm.DerivePlanViewWithSupersessions(tasks, edges, records, outcomes, paused, supersessions)}, nil
}
...
func (s *Service) planSummaries(ctx context.Context, projectID pm.ProjectID, includeArchived bool, limit, offset int) ([]*PlanDetail, int, error) {
	...
	allSupersessions, err := s.plans.ListSupersessionsByPlans(ctx, planIDs)
	if err != nil {
		return nil, 0, err
	}
	supersessionsByPlan := make(map[pm.PlanID][]pm.PlanSupersession, len(plans))
	for _, s := range allSupersessions {
		supersessionsByPlan[s.PlanID] = append(supersessionsByPlan[s.PlanID], s)
	}
	...
	for _, p := range plans {
		...
		view := pm.DerivePlanViewWithSupersessions(tasks, edgesByPlan[p.ID()], recordsByPlan[p.ID()], outcomesByPlan[p.ID()], paused, supersessionsByPlan[p.ID()])
