	...
	for _, status := range pm.AllTaskStatuses() {
		...
		t.Run(string(status), func(t *testing.T) {
			...
			if !errors.As(err, &nodeErr) {
				t.Fatalf("remove %s err=%v, want PlanNodeNotRemovableError", status, err)
			}
			if nodeErr.CurrentStatus != status || !hasStringPrefix(nodeErr.HistoryBlockers, "task_status:"+string(status)) {
				t.Fatalf("node error=%+v, want status blocker %s", nodeErr, status)
			}
		})
	}
}
...
func TestRemoveTaskFromPlan_RejectsStartedResetNode(t *testing.T) {
	...
	if _, err := h.svc.db.ExecContext(h.ctx, `UPDATE pm_tasks SET execution_lease_expires_at=? WHERE id=?`,
		expired.Format(time.RFC3339Nano), string(taskA)); err != nil {
		t.Fatal(err)
	}
	...
	if !hasStringPrefix(nodeErr.HistoryBlockers, "task_action_log:agent_started") ||
		!hasStringPrefix(nodeErr.HistoryBlockers, "task_action_log:reset") {
		t.Fatalf("history blockers = %v, want agent_started and reset", nodeErr.HistoryBlockers)
	}
	tk, _ := h.tasks.FindByID(h.ctx, taskA)
	if tk.PlanID() != planID {
		t.Fatalf("rejected remove changed plan_id to %q", tk.PlanID())
	}
}
...
func TestRemoveTaskFromPlan_RejectsGateAndAcceptanceHistory(t *testing.T) {
	...
	if err := h.plans.RecordReviewVerdict(h.ctx, planID, pm.ReviewVerdict{
		PlanID: planID, TaskID: review, Verdict: "pass", SHA: "abc123",
	}, h.clk.Now()); err != nil {
		t.Fatal(err)
	}
	if err := h.svc.remediation.SaveVerdict(h.ctx, pm.GateVerdict{
		ID: "gv-1", ProjectID: pid, PlanID: planID, GateTaskID: gate,
		Outcome: pm.GateVerdictPass, Evidence: "ok", ReviewedSHA: "abc123",
		ActorRef: "user:a", IdempotencyKey: "gate-key", CreatedAt: h.clk.Now(),
	}); err != nil {
		t.Fatal(err)
	}
	for _, tc := range []struct {
		...
		{gate, "gate_verdict:gv-1"},
	} {
		err := h.svc.RemoveTaskFromPlan(h.ctx, planID, tc.task, "user:a")
		var nodeErr *pm.PlanNodeNotRemovableError
		if !errors.As(err, &nodeErr) {
			t.Fatalf("remove %s err=%v, want PlanNodeNotRemovableError", tc.task, err)
		}
		if !hasStringPrefix(nodeErr.HistoryBlockers, tc.blocker) {
			t.Fatalf("remove %s err=%v blockers=%v, want %s", tc.task, err, nodeErr.HistoryBlockers, tc.blocker)
		}
		tk, _ := h.tasks.FindByID(h.ctx, tc.task)
		if tk.PlanID() != planID {
			t.Fatalf("rejected remove changed plan_id for %s to %q", tc.task, tk.PlanID())
		}
	}
	if _, ok, err := h.plans.GetReviewVerdict(h.ctx, planID, review); err != nil || !ok {
		t.Fatalf("review verdict lost ok=%v err=%v", ok, err)
	}
	if verdicts, err := h.svc.remediation.ListVerdictsByPlan(h.ctx, planID); err != nil || len(verdicts) != 1 {
		t.Fatalf("gate verdict history = %v err=%v, want one retained", verdicts, err)
	}
}
...
func TestRemoveTaskFromPlan_ConcurrentStartAtMostOneWins(t *testing.T) {
	for i := 0; i < 20; i++ {
		...
		go func() {
			defer wg.Done()
			startErr = h.svc.StartTask(h.ctx, taskA, "user:a")
		}()
		go func() {
			defer wg.Done()
			removeErr = h.svc.RemoveTaskFromPlan(h.ctx, planID, taskA, "user:a")
		}()
		wg.Wait()
		if startErr == nil && removeErr == nil {
			t.Fatalf("iteration %d: StartTask and RemoveTaskFromPlan both succeeded", i)
		}
		tk, _ := h.tasks.FindByID(h.ctx, taskA)
		if tk.PlanID() == "" && tk.Status() == pm.TaskRunning {
			t.Fatalf("iteration %d: node was removed and started: status=%s plan=%q", i, tk.Status(), tk.PlanID())
		}
	}
}
