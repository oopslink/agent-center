...
func TestEditPlanTopology_RemoveNodeRequiresZeroHistoryAndRollsBackBatch(t *testing.T) {
	...
	base := h.planVersion(t, planID)
	_, err := h.edit(t, planID, base, TopologyOp{Kind: OpRemoveNode, TaskID: removable}, TopologyOp{Kind: OpRemoveNode, TaskID: blocked})
	var nodeErr *pm.PlanNodeNotRemovableError
	if !errors.As(err, &nodeErr) || nodeErr.TaskID != blocked || !hasStringPrefix(nodeErr.HistoryBlockers, "dependency:") {
	...
	for _, id := range []pm.TaskID{removable, blocked, dep} {
		...
		if tk.PlanID() != planID {
			t.Fatalf("task %s plan=%q", id, tk.PlanID())
		}
	}
}
