...
func TestGetPlanGenerations_ReadsPersistedG0GnSnapshotsAndOwnership(t *testing.T) {
	...
	if len(evolved.Generation.Snapshot.Tasks) != 3 || len(evolved.Generation.Diff.Tasks) != 1 || len(evolved.Generation.Diff.NodeDecisions) != 2 || len(evolved.Generation.Diff.Edges) != 1 {
