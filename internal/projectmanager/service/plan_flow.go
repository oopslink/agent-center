...
func (s *Service) requirePlanNodeRemovable(ctx context.Context, p *pm.Plan, t *pm.Task) error {
	...
	// Current dependency edges are topology, not execution history. A node that
	// has never started or dispatched remains removable from a pending Plan; the
	// same transaction removes all of its incident edges before membership is
	// cleared. Treating those edges as history would make ordinary pending DAG
	...
	if _, ok, err := s.plans.GetBlockedOn(ctx, p.ID(), t.ID()); err != nil {
		return err
	} else if ok {
		blockers = append(blockers, "blocked_on_snapshot")
	}
	...
	if len(blockers) > 0 {
		return &pm.PlanNodeNotRemovableError{
			...
			AllowedStatus: "pending", HistoryBlockers: blockers,
		}
	}
	return nil
}
