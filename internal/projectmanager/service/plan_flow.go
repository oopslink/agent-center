...
func (s *Service) requirePlanNodeRemovable(ctx context.Context, p *pm.Plan, t *pm.Task) error {
	...
	if edges, err := s.plans.ListDependencies(ctx, p.ID()); err != nil {
		return err
	} else {
		for _, e := range edges {
			if e.FromTaskID == t.ID() || e.ToTaskID == t.ID() {
				blockers = append(blockers, fmt.Sprintf("dependency:%s->%s", e.FromTaskID, e.ToTaskID))
			}
		}
	}
