...
func (s *Service) reconcilePlanProgress(ctx context.Context, planID pm.PlanID, opt progressReconcileOptions) error {
	return s.reconcilePlanProgressWithFence(ctx, planID, pm.ProgressFence{}, opt)
}

func (s *Service) reconcilePlanProgressWithFence(ctx context.Context, planID pm.PlanID, fence pm.ProgressFence, opt progressReconcileOptions) error {
	if s.plans == nil {
	...
	return s.runInTx(ctx, func(txCtx context.Context) error {
		for _, t := range tasks {
			v := s.evaluateTaskProgress(txCtx, p, t, dispatched[t.ID()], blockedByTask[t.ID()], coverage, now, opt)
			var obligations []pm.ProgressObligation
			var incidents []pm.ProgressIncident
			var resolved []pm.ProgressObligationResolution
			for _, fact := range v.Facts {
				switch fact.Summary {
				case "watermark_lag":
					incidents = append(incidents, progressIncident(v, pm.IncidentWatermarkLag, fact.ID, now))
				case "persistent_suspect":
					incidents = append(incidents, progressIncident(v, pm.IncidentProgressClassificationUnknown, fact.ID, now))
				case "missing_progress_contract":
					incidents = append(incidents, progressIncident(v, pm.IncidentMigrationGap, fact.ID, now))
				}
			}
			if v.Quality == pm.ProgressQualitySuspect && v.SuspectKey != "" {
				obligations = append(obligations, progressObligation(v, pm.ObligationSourceRecovery, now))
			} else {
				resolved = append(resolved, pm.ProgressObligationResolution{
					PlanID: v.PlanID, TaskID: v.TaskID, Kind: pm.ObligationSourceRecovery,
					FactRef: progressFactID(v.PlanID, v.TaskID, "source_recovered", strconv.Itoa(t.Version())),
					Actor:   "system", At: now,
				})
			}
			if t.Status() == pm.TaskRunning && t.Delivery() == nil {
				obligations = append(obligations, progressObligation(v, pm.ObligationProduceDelivery, now))
			}
			if err := s.plans.SaveProgressEvaluation(txCtx, fence, v, obligations, incidents, resolved); err != nil {
				return err
			}
		}
		if err := s.reconcilePlanLevelProgress(txCtx, p, tasks, blocked, now); err != nil {
			return err
		}
		return nil
	})
