...
func TestProgressControl_SourceRecoveryClosesObligationAfterAuthoritativeFact(t *testing.T) {
	h, planID, taskID := progressPlanWithOneTask(t)
	if err := h.svc.reconcilePlanProgress(h.ctx, planID, progressReconcileOptions{WatermarkLagSLA: time.Minute, SourceGraceCycle: 1, SuspectMaxCycles: 2}); err != nil {
		t.Fatalf("reconcile suspect: %v", err)
	}
	if obs, _ := h.plans.ListOpenProgressObligations(h.ctx, planID); obligationsOfKind(obs, pm.ObligationSourceRecovery) != 1 {
		t.Fatalf("source_recovery obligations before recovery=%+v, want one", obs)
	}
	if err := h.plans.RecordDispatch(h.ctx, planID, taskID, h.clk.Now(), "dispatch-authority"); err != nil {
		t.Fatalf("record dispatch authority: %v", err)
	}
	h.clk.Advance(time.Second)
	if err := h.svc.reconcilePlanProgress(h.ctx, planID, progressReconcileOptions{WatermarkLagSLA: time.Minute, SourceGraceCycle: 1, SuspectMaxCycles: 2}); err != nil {
	...
	got, ok, err := h.plans.LatestProgressObservation(h.ctx, planID, taskID)
	if err != nil || !ok {
		t.Fatalf("latest recovered = (%+v,%v,%v), want row", got, ok, err)
	}
	if got.Quality != pm.ProgressQualityValid || got.SuspectKey != "" {
		t.Fatalf("recovered observation=%+v, want valid audit fact with no suspect", got)
	}
	if obs, _ := h.plans.ListOpenProgressObligations(h.ctx, planID); obligationsOfKind(obs, pm.ObligationSourceRecovery) != 0 {
		t.Fatalf("source_recovery obligation still open after recovery: %+v", obs)
	}
}
