...
func TestEvolvePlanGeneration_OwnerSupersedesCompletedTerminalNodeWithoutRemoveGate(t *testing.T) {
	...
	if reloadedA.PlanID() != planID {
		t.Fatalf("superseded completed task plan=%s, want immutable history in %s", reloadedA.PlanID(), planID)
	}
	if reloadedA.NodeID() != "" {
		t.Fatalf("superseded completed task kept stale node_id=%s", reloadedA.NodeID())
	}
	if backlog, err := h.svc.ListUnplannedTasks(h.ctx, pid); err != nil {
		t.Fatal(err)
	} else {
		for _, task := range backlog {
			if task.ID() == a {
				t.Fatalf("superseded completed task %s leaked into backlog", a)
			}
		}
	}
...
func TestEvolvePlanGeneration_OwnerSupersedesDiscardedTerminalNode(t *testing.T) {
	...
	if reloadedA.PlanID() != planID {
		t.Fatalf("superseded discarded task plan=%s, want immutable history in %s", reloadedA.PlanID(), planID)
	}
	if reloadedA.NodeID() != "" {
		t.Fatalf("superseded discarded task kept stale node_id=%s", reloadedA.NodeID())
	}
	if backlog, err := h.svc.ListUnplannedTasks(h.ctx, pid); err != nil {
		t.Fatal(err)
	} else {
		for _, task := range backlog {
			if task.ID() == a {
				t.Fatalf("superseded discarded task %s leaked into backlog", a)
			}
		}
	}
	if oldA.NodeID() != "" {
