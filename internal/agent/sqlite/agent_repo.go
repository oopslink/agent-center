...
func (r *AgentRepo) Save(ctx context.Context, a *agent.Agent) error {
	...
	_, err = exec.ExecContext(ctx,
		`INSERT INTO agents (id, organization_id, name, description, model, cli, reasoning, mode, provider,
			...
			capability_tags, auto_assignable, include_description_in_system_prompt, judge_enabled, executor_git_worktree, sandbox_enabled, sandbox_provider, worker_id, lifecycle, lifecycle_error, created_by, identity_member_id, created_at, updated_at, last_lifecycle_transition_at, version)
		 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
		...
		env, tags, boolToInt(p.AutoAssignable), boolToInt(p.IncludeDescriptionInSystemPrompt), boolToInt(p.JudgeEnabled), boolToInt(p.ExecutorGitWorktree), boolToInt(p.SandboxEnabled), nullString(p.SandboxProvider), a.WorkerID(), string(a.Lifecycle()), nullString(a.LifecycleError()),
...
func (r *AgentRepo) Update(ctx context.Context, a *agent.Agent) error {
	...
	res, err := exec.ExecContext(ctx,
		`UPDATE agents SET name=?, description=?, model=?, cli=?, reasoning=?, mode=?, provider=?,
			orchestrator_model=?, default_executor_model=?, max_concurrent_tasks=?, allowed_models=?, allowed_executors=?, env_vars=?,
			capability_tags=?, auto_assignable=?, include_description_in_system_prompt=?, judge_enabled=?, executor_git_worktree=?, sandbox_enabled=?, sandbox_provider=?, lifecycle=?, lifecycle_error=?, updated_at=?, last_lifecycle_transition_at=?, version=? WHERE id=?`,
		...
		tags, boolToInt(p.AutoAssignable), boolToInt(p.IncludeDescriptionInSystemPrompt), boolToInt(p.JudgeEnabled), boolToInt(p.ExecutorGitWorktree), boolToInt(p.SandboxEnabled), nullString(p.SandboxProvider), string(a.Lifecycle()), nullString(a.LifecycleError()), ts(a.UpdatedAt()), ts(a.LastLifecycleTransitionAt()), a.Version(), string(a.ID()))
	if err != nil {
		return err
	}
...
const agentSelect = `SELECT id, organization_id, name, description, model, cli, reasoning, mode, provider,
	orchestrator_model, default_executor_model, max_concurrent_tasks, allowed_models, allowed_executors, env_vars,
	capability_tags, auto_assignable, include_description_in_system_prompt, judge_enabled, executor_git_worktree, sandbox_enabled, sandbox_provider, worker_id, lifecycle, lifecycle_error, created_by, identity_member_id, created_at, updated_at, last_lifecycle_transition_at, version FROM agents`
...
func scanAgent(scan func(...any) error) (*agent.Agent, error) {
	var (
		...
		sandboxProvider                                                     sql.NullString
		version                                                             int
	)
	if err := scan(&id, &org, &name, &desc, &model, &cli, &reasoning, &mode, &provider,
		...
		&tagsJSON, &autoAssignable, &includeDescInPrompt, &judgeEnabled, &executorGitWorktree, &sandboxEnabled, &sandboxProvider, &workerID, &lifecycle, &lifecycleErr, &createdBy, &identityMemberID, &createdAt, &updatedAt, &lastLifecycleTransitionAt, &version); err != nil {
		return nil, err
	}
	...
	return agent.RehydrateAgent(agent.RehydrateAgentInput{
		...
		Profile: agent.Profile{
			...
			SandboxEnabled:      sandboxEnabled.Valid && sandboxEnabled.Int64 != 0,
			SandboxProvider:     sandboxProvider.String,
		},
