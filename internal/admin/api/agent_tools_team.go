...
func (s *Server) getTeamRuleHandler(w http.ResponseWriter, r *http.Request) {
	...
	if d.TeamRuleAuditRepo == nil {
		writeError(w, http.StatusNotImplemented, "team_rule_audit_not_wired", "team rule audit repository is not wired on this center")
		return
	}
	teamID, hasTeam, ok := s.resolveRuleTeam(w, r, d, a)
	if !ok {
	...
	_, err = d.TeamRuleAuditRepo.AppendLoaded(r.Context(), ruleregistry.LoadAudit{
