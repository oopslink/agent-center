...
func TestReportDeliveryReq_DecodesAndMapsPushError(t *testing.T) {
	body := `{"agent_id":"a","task_id":"t","source":"manual_recovery","executor_id":"exec-dead","evidence":"go test ./... PASS","reason":"executor exhausted","git":{"branch":"ac-exec/t/e","head_sha":"deadbeef","dirty":true,"dirty_paths":["dirty.txt"],"worktree":"/tmp/w","probed":true,"pushed":false,"base_ref":"main","base_known":true,"ahead_of_base":1,"push_error":"remote rejected (pre-receive hook declined)"}}`
	...
	g := req.Git
	d := &pm.Delivery{
		Branch: g.Branch, HeadSHA: g.HeadSHA, Dirty: g.Dirty, DirtyPaths: append([]string(nil), g.DirtyPaths...), Pushed: g.Pushed, Probed: g.Probed,
		BaseRef: g.BaseRef, BaseKnown: g.BaseKnown, AheadOfBase: g.AheadOfBase, PushError: g.PushError,
		Source: req.Source, ExecutorID: req.ExecutorID, Worktree: g.Worktree, Evidence: req.Evidence, Reason: req.Reason,
	}
	...
	if d.Worktree != "/tmp/w" || len(d.DirtyPaths) != 1 || d.DirtyPaths[0] != "dirty.txt" {
		t.Fatalf("dirty path/worktree metadata dropped: %+v", d)
	}
}
