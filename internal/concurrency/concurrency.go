...
// SlotSnapshot is one addressable runtime slot in an AgentSnapshot. Occupied
// slots mirror the executor fields the UI needs; empty slots carry only
// slot_index+state.
...
// AgentSnapshot is one agent's live executor set at heartbeat time. Active is
// the count of slot-occupying executors. The cap + queued depth are joined
// center-side (they are not the worker's to know), while admission/slot/config
// fields are the runtime's own view and may lag the center profile during
// mixed-version config propagation.
