...
| Field | Value |
|---|---|
| Status | Accepted (Refined by [ADR-0018](0018-detached-agent-via-per-execution-shim.md)) |
| Date | 2026-05-16 |

> **Refinement note**：本 ADR § Decision 3 描述的"Worker 本地 sqlite ledger"在 [ADR-0018](0018-detached-agent-via-per-execution-shim.md) 中被改写为"per-execution 目录 + events.jsonl"。本 ADR 的其他决策（execution_id 幂等 / ACK 协议 / Reconcile / 双跑兜底）全部保留。下文 § 3 段保留为历史记录，落地以 ADR-0018 § 3 为准。
...
- Worker 接收 envelope 用 `execution_id` 做幂等：重复收到同 id → 检查本机状态决定 ACK / 忽略（v1 实际承载介质见 [ADR-0018](0018-detached-agent-via-per-execution-shim.md)）
...
### 3. Worker 本地持久化（→ ADR-0018 改写为 per-execution 目录）

**原方案（已被 [ADR-0018](0018-detached-agent-via-per-execution-shim.md) § 3 替代）**：worker daemon 维护一个 sqlite ledger，收到 envelope 先写 ledger 再 ACK，重启后扫表恢复。

**现行方案**：worker 在 `~/.agent-center-worker/exec/<execution_id>/` 下维护 per-execution 目录（envelope.json / status.json / events.jsonl / agent.log / shim.sock），daemon 收 envelope 后先写 `envelope.json` 再 ACK；shim 写 `status.json` 维护 phase；重启时扫 `exec/*/status.json` 决定每个 execution 的去向。详见 [ADR-0018 § 3 / § 5](0018-detached-agent-via-per-execution-shim.md)。

幂等查询（语义不变，存储介质变了）：

| 本地状态 | Worker 行为 |
|---|---|
| 无 `exec/<id>/` 目录 | 创建目录 + 写 envelope.json → ACK → spawn shim |
...
场景：Worker A 心跳超 60s（见 [task-runtime/02-task-execution.md § timeout](../architecture/tactical/task-runtime/02-task-execution.md) `worker_heartbeat_timeout`）→ Center 标 W-A.status=offline + 上面所有 active execution → `failed(reason='worker_lost')` → supervisor 派给 W-B（新 execution_id）。
...
- **Worker 引入本地持久化**（[ADR-0018](0018-detached-agent-via-per-execution-shim.md) 后细化为 per-execution 目录）：增加 worker daemon 复杂度；接受 —— 避免"daemon 崩溃丢派单"风险
...
- 新增 [task-runtime/02-task-execution.md](../architecture/tactical/task-runtime/02-task-execution.md)：dispatch ACK + Reconcile + ledger 章节（按 [ADR-0019](0019-bc-scheduling-execution-merged-to-task-runtime.md) 合并；曾在 scheduling/01-task-model）
