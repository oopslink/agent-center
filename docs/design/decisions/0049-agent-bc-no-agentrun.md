...
### 3. AgentWorkItem 状态机（plan § 2.4，修订 2026-05-29 plan §10 OQ11：**无 blocked 态**）

```text
queued → active → waiting_input → active
active → done
active → failed
queued/active/waiting_input → canceled
queued/active/waiting_input → superseded
```

- **"blocked" 只是 Task（业务）概念，AgentWorkItem 没有 blocked 态**（plan §10 OQ11）。
- Task 被 BlockTask → 取消对应 live WorkItem（`canceled`，本次尝试结束，Agent 转空闲）；Task 解阻塞（blocked→running）→ **新建** WorkItem。
- `superseded` = 重派（assignee 变更：旧→superseded + 新建）；`canceled` = Task 阻塞/取消/显式终止；`failed` = 尝试出错；`waiting_input` = 等回答（可续，OQ5）。
...
- StopAgent 是操作态，**不**触碰 WorkItem（WorkItem 无 blocked 态；Task 的 blocked 才是显式业务态，需消息/原因，且不下沉到 WorkItem）。
