...
[02-task-model.md § 12.1](../architecture/02-task-model.md) 和 [06 当前 TBD-partial](../architecture/06-supervisor-model.md) 都提到了"每个聚合根维持 30 秒 coalescing window，同源事件合并成一个 batch 喂给 supervisor"，但没明确：**当两个不同聚合根的窗口同时就绪时，supervisor 进程是串行跑还是并行 spawn？**
...
- 重写 [architecture/06-supervisor-model.md](../architecture/06-supervisor-model.md) § 2 Invocation 生命周期 + § 3 调度 / 节流（TBD-partial → Draft）
- 更新 [architecture/01-bounded-contexts.md § BC5](../architecture/01-bounded-contexts.md)：核心事件加 `supervisor.invocation_failed_alert`
- 更新 [architecture/02-task-model.md § 12.1](../architecture/02-task-model.md)：唤醒事件白名单跟 06 § 3 引用对齐
- 更新 [architecture/05-observability.md § O3](../architecture/05-observability.md)：Supervisor 调用全留档章节补充并发 / 重试 / crash 字段说明
