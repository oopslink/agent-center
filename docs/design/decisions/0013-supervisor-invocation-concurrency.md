...
[02-task-model.md § 12.1](../architecture/tactical/scheduling/01-task-model.md) 和 [06 当前 TBD-partial](../architecture/tactical/cognition/01-supervisor-model.md) 都提到了"每个聚合根维持 30 秒 coalescing window，同源事件合并成一个 batch 喂给 supervisor"，但没明确：**当两个不同聚合根的窗口同时就绪时，supervisor 进程是串行跑还是并行 spawn？**
...
- 更新 [architecture/01-bounded-contexts.md § BC5](../architecture/strategic/03-bounded-contexts.md)：核心事件加 `supervisor.invocation_failed_alert`
- 更新 [architecture/02-task-model.md § 12.1](../architecture/tactical/scheduling/01-task-model.md)：唤醒事件白名单跟 06 § 3 引用对齐
