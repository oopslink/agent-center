...
[ADR-0015](0015-agent-trace-not-in-events-table.md) 让 worker 进度数据由 TaskExecution 投影（实时摘要）+ BlobStore 归档（post-hoc）+ worker peek-trace RPC（按需深挖）三层承载。但这些都是"数据可拿到"，**没解决"~~飞书~~ vendor 侧用户怎么看见 worker 工作进度"**（v2 删 vendor 集成 per ADR-0031）：

- ~~09-feishu-integration.md § 5~~ (~~tactical/bridge/* (v2 deleted per ADR-0031)~~) 的 outbound 触发器只订阅 `conversation.message_added` / `issue.comment_added` / `issue.opened`
- ~~TaskExecution 投影变化（`current_activity` 变了）**不是 domain event** → Bridge 不会自动推送~~ (v2 删 Bridge per ADR-0031)
- 用户想"持续看进度"只能反复 @bot 问 → 每次烧一次 supervisor invocation 回静态卡 —— 不可接受

讨论中识别出可行模式有三档：

- ~~**B (update_card refresh)**：Bridge 周期调飞书 `update_card` API 重渲染同一张卡片~~ (v2 删 Bridge / vendor per ADR-0031)
- **B' (bound thread + 进度消息流)**：把卡片绑到 thread，后续进度作为 thread 内 message 追加
- **B'' (混合)**：主卡片 update + 关键 milestone 作消息

~~09 § 7 Bound Card~~ (~~tactical/bridge/* (v2 deleted per ADR-0031)~~) 已经在 Issue 上跑通 bound card 模式（卡片 + thread + thread 内消息=IssueComment）。把同样机制扩展到 Task 复用度高、用户认知一致。
...
- ~~新 CLI `agent-center task bind-card <task_id> --channel=feishu --auto`（同构于 `issue bind-card`）~~ (v2 删 vendor-specific CLI per ADR-0031)

### 2. 复用 Issue bound_card 机制 ~~(v2 删 Bridge per ADR-0031)~~
...
- 进度消息**写入 messages 表**（~~保 09 § 12 "飞书是无状态通道；thread 内每条消息都同步写入领域模块" 承诺~~ — tactical/bridge/* v2 deleted per ADR-0031）
- 默认 `inspect conversation <id>` 时过滤掉 `task_progress`（不污染对话历史浏览）
- 保留期可短（默认 30 天 GC，可配，独立于普通 message 的保留期）

### 5. 显式驳回 B（update_card 周期重渲染） ~~(v2 删 Bridge / vendor per ADR-0031)~~
...
- ~~**天然时间线**：飞书 thread 本身就是历史可回滚视图~~ (v2 删 vendor per ADR-0031)
...
### B. update_card 周期重渲染同一张卡片 ~~(v2 删 Bridge / vendor per ADR-0031)~~
...
- Pro: 复用现有机制；天然时间线；~~零新 Bridge 自治~~ (v2 删 Bridge per ADR-0031)
...
- 改写 [task-runtime/01-task.md](../architecture/tactical/task-runtime/01-task.md)：
  ...
  - ~~新增 CLI：`agent-center task bind-card <task_id> --channel=feishu --auto` / `task unbind-card <task_id>`~~ (v2 删 vendor-specific CLI per ADR-0031)
- ~~改写 09-feishu-integration.md (tactical/bridge/* v2 deleted per ADR-0031)：~~
