...
新增的 failed reason 进 [02-task-model.md § 3.6](../architecture/02-task-model.md)：`shim_no_hello` / `shim_crashed`。
...
| Worktree `<base_path>.wt/task-<execution_id>` | 24h | 现有行为，[07-worker-model.md § Workspace](../architecture/07-worker-model.md) |
...
- 改写 [07-worker-model.md](../architecture/07-worker-model.md)：
...
- 微调 [05-observability.md](../architecture/05-observability.md)：
