...
构建一个 **agent 调度中心**，统一管理用户在多台机器上的 agent CLI 工具（claude code、codex、opencode 等），通过飞书入口接收任务、按项目维度归集、按任务粒度调度。
...
| 飞书集成 | 一键创建应用 + WebSocket 长连接（飞书侧零暴露） |
