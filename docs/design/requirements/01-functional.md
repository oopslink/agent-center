...
> 🆕 **v2 更新**（[ADR-0031](../decisions/drafts/0031-v2-drop-bridge-vendor-integration.md) Drop Bridge + [ADR-0037](../decisions/drafts/0037-web-console-as-main-user-ui.md) W1 + [ADR-0038](../decisions/drafts/0038-cli-ux-enhancement.md) W2）：v1 中 F1 / F6 / F14 / F16 / F18 等含飞书 vendor 描述，v2 已撤回；用户主入口改为 **Web Console + CLI**。

| ID | 需求 |
|---|---|
| F1 | 通过 **Web Console / CLI** 接收任务请求（v2: vendor 撤回，per [ADR-0031](../decisions/drafts/0031-v2-drop-bridge-vendor-integration.md)）|
...
| F6 | Issue 支持多轮讨论：Conversation Message 时间线承载（kind=issue Conversation；v2 [ADR-0039](../decisions/drafts/0039-conversation-business-model-v2-unified.md)）；用户 / Supervisor / Worker agent 参与 |
...
| F14 | 任意任务可实时 tail 日志（CLI `conversation tail -f` per [ADR-0038](../decisions/drafts/0038-cli-ux-enhancement.md) + Web Console SSE per [ADR-0037](../decisions/drafts/0037-web-console-as-main-user-ui.md)）|
...
| F21 | 提供本地 **Web Console UI**（嵌入 `agent-center server` 进程），仅本机绑定（loopback），用户通过 SSH 隧道访问。**v2 升级为全量用户 UI**（聊天 / 看 channel / 回 InputRequest / 发起任务 / 派生 Issue/Task / fleet view / trace 等）；技术栈 Go API + SPA + SSE，per [ADR-0037](../decisions/drafts/0037-web-console-as-main-user-ui.md)。SSH 隧道由用户自理（不在本项目范围）|
