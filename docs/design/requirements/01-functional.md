...
| F30 | **Worker 重启/部署不打断在飞 agent 工作（survive + re-attach，v2.7）**：运维重启/部署一个 worker 时，该 worker 上**在飞的 agent 任务继续、不中断、不丢、执行/副作用恰一次**（agent 进程存活、worker 重新接上；非杀+重起）。整机重启 / agent 自崩 = 真死 → 重启恢复 + 续会话（短暂恢复、非零中断）。**产品行为与 e2e 验收源详见 [survive-reattach 产品行为说明](survive-reattach-product-behavior.md)**（e2e 以该说明为准、dev 禁入；进程级"同一进程没死"机制断言归集成层）。详见 [v2.7 plan §5 D](../../plans/v2.7-domain-refactor-plan.md)。 |
