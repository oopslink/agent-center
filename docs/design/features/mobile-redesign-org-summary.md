...
| Status | Proposed |
| Date | 2026-07-16 |
| Scope | Workspace 模块的跨项目视图：OrgWorkItems（`/issues` `/tasks`，同一组件两种 kind）/ OrgPlans（`/plans`）/ OrgRepos（`/repos`，工作区仓库注册表）/ OrgModelCatalog（`/model-catalog`） |
...
第五批交付物，仍在底部 Tab「Work」下，但是与第三批（ProjectDetail 内的项目内视图）平级的**跨项目**视图——PC 端顶层路由 `/issues` `/tasks` `/plans` `/repos` `/model-catalog`，对应 Workspace col② 导航。

审计澄清了容易混淆的概念，写这批 spec 前必须先分清楚：
...
2. **Team Memory rules 不属于 Workspace 跨项目导航**。规则文件只在 Team Detail > Memory 的 `rules/` 分组内查看和管理，不再通过 Workspace `/templates` 或 Team `/teams/templates` 独立页面承载。
...
| OrgModelCatalog | `/model-catalog` | 注册表 | 组织级模型类目 |

Workspace 顶层 `tabstrip` 从第三批的"Projects"单项扩展为 **Projects / Issues / Tasks / Plans / Repos / Model catalog**，横向可滑动——对应 PC 端 Workspace col② 集合，与第三批 ProjectDetail 内部的 tabstrip 是同一视觉语言但不同语义层级（这里切的是"跨项目视图"，那里切的是"单项目内的维度"）。
...
- 模型类目保留为 Workspace 下的组织级注册表入口。
- Team rules 不在此处建二级入口；移动端进入 Team Detail > Memory 后用 `entries/` 与 `rules/` 分组承载同一产品面。
...
| Team Memory rules 查看/管理入口 | `MemoryPane` | N/A in Workspace — 在 Team Detail > Memory 的 `rules/` 分组内承载，不新增 Workspace 页 |
...
- Team Memory rules 的编辑表单细节；Web 当前提供查看、分组筛选、权限反馈与 git 管理入口。
