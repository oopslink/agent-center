...
| 包 / 文件 | 行覆盖 | 说明 |
|---|---|---|
| `internal/workerdaemon/orchestrator`（engine/runner/idminter）| **97.0%** | 仅 `HandleWork` 的 merge-error 分支（§17 Launched+err）未覆——单写者下 Merge 不会失败，防御性不可确定触发 |
| `internal/workerdaemon/executor`（含 pool.go worktree 可选改造）| **94.0%** | F2/F1 既有 + W1 plain-dir 改造；未覆为既有深层 IO 防御分支 |
| `internal/workerdaemon/concurrent_exec.go`（接线）| 见下 | 可单测面已覆；残余为防御 / 集成-only |

`concurrent_exec.go` + `maybeAttachExecutorEngine` 逐函数：`funcClock.Now` / `concurrencyEnabled` /
`firstNonEmptyLine` **100%**；`workViaExecutor` **94.7%**；`maybeAttachExecutorEngine` **86.7%**；
`buildExecutorEngine` **75%**；`drainExecutor` **75%**。

**显式标注「集成 / Accept-only 或防御-only」（不计入可单测门）**：
- `buildExecutorEngine` 残余 = `NewFileExchange/NewPool/NewRoutingStore/NewEngine/NewMonitor` 的
  `return nil, err` 防御分支——plain-dir 合法输入下不可达（空 root 分支已测）。
- `drainExecutor` 残余 = `Monitor.AwaitCompletion` 返回错误时的 log 分支——真 `true` 进程下不触发；
  真失败路径属 Accept（真 claude executor 异常退出）。
- `maybeAttachExecutorEngine` 残余 = `buildExecutorEngine` 失败的 fallback-log 分支（路径错误已测）。
- 真 fork→真 claude 执行链整体属 **Accept 端到端**（在测试 agent 开并发 profile 实跑），单测以
  真 `true` harness 验证「fork→drain→释放槽」与串链/路由/选模/落盘。
