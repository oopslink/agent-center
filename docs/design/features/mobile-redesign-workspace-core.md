...
第三批交付物，覆盖底部 Tab「Work」（不是 Chat——Work 对应 Projects/Issues/Tasks/Plan/Repos，Chat 对应上一批的 Conversations，两者是并列模块，命名已在批次一/二混淆过，这里再次明确）。2026-08 收口后，Workspace 不再暴露独立 Templates 页面。
...
- Workspace Templates 页面已 retired；team rules 统一在 Team Detail > Memory 的 `rules/` 分组查看，不再作为 Workspace 一级路由。
