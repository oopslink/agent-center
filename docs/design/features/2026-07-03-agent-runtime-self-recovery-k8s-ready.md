...
### 4.5 worker = controller：AgentLauncher 抽象 + 崩溃=退出重建（范围甲）
oopslink 定：**k8s 下 agent pod 由 worker pod 创建**。单机同构 = worker 进程 spawn agent 进程。故 worker/daemon 从"进程内托 N 个 runtime"重构成 **launcher/controller**：按中心的 agent 清单，**创建 / 监视 / 重建** agent 单元。
- **`AgentLauncher` 抽象**（worker→agent 的创建/重建）：
  - **`LocalProcessLauncher`（本次做）**：worker 进程 fork/exec 一个 `cmd/agent-runtime` 进程；退出/崩溃 worker 重新拉起。
  - **`K8sPodLauncher`（推迟）**：worker pod 用 k8s API 创建/重建 agent pod。
  - 同一接口，worker 只管"这个 agent 该在 → 确保它跑着"，创建机制交给 launcher。
- **崩溃模型统一**：每个 agent 是独立进程/pod（由 worker 创建）→ 不可恢复崩溃 = **agent 进程退出 → worker（单机 LocalProcessLauncher）/ k8s 重新创建**。因每 agent 独立进程，退出只影响自己——**不再需要"进程内可插拔崩溃 seam"**（范围乙下才有的顾虑，甲下消失）。
...
**本次做**（含独立入口——它是自包含的强制验证器、单机就能跑、不需要 k8s）：
- 独立 `cmd/agent-runtime` main（config→center client→runtime→Boot 自恢复→tick 循环[`Tick`/`RunWatchdog`/lease/GC/heartbeat]→命令入口→优雅 drain）。
- `AgentLauncher` 接口 + `LocalProcessLauncher`；worker/daemon 重构成 launcher/controller（§4.5）。
- 单机即以"worker 进程 + 每 agent 一个 `cmd/agent-runtime` 进程"跑起来（目标拓扑，只差 k8s 的 pod 调度/PV）。

**推迟**（真上 k8s 才需要）：`K8sPodLauncher`（k8s API 建 pod）+ 持久卷 PV/StorageClass 声明 + k8s manifest。持久化目录（home/`executors/`/`tasks/`/`repos/` 跨重启）本次即须成立（单机=本地目录，k8s=PV），只是 PV 声明推迟。
...
- **D1 [Dev] 去共享状态**（§4.1，前提）+ 单测。
- **D2 [Dev] runtime 自建 center 连接 + list-my-inflight-tasks（不过滤全量在途）**（§4.2，依赖 D1）+ 测。
- **D3 [Dev] executor session-id 持久化 + runner 可 --resume + 三档恢复阶梯**（§4.3，依赖 D1）+ 测。
- **D4 [Dev] runtime 自触发 boot reconcile（自恢复）**（§4.4，依赖 D2、D3）+ 测。
- **D5 [Dev] 控制/观测硬化：即时死亡 + stall 阈值 + 卡/死→自动接恢复链 + 事件流自恢复后不断**（§4.6，依赖 D4）+ 测。
- **D6 [Dev] AgentLauncher + LocalProcessLauncher + cmd/agent-runtime 入口 + worker=controller**（§4.5/§5，依赖 D5——agent 进程内要有自包含自恢复的 runtime）+ 测。
- **[验收·真跑]**（依赖 D6）：§6 全部七项。
- **[集成] → [Gate·PD] → [Ship]**。
每个 D 配独立**复审**节点 + **决策**节点（pass 放行下游 / reject 有界 loopback 回该 Dev，max_rounds=2）。
