...
- Agent 持久记忆（`memory/` scoped 多文件，git 管理）存放经验、教训、技能、原则；业务实体 ground truth 在 Center
...
| **Memory** | Cognition | file-based + git 仓；**scoped 多文件**（global `CLAUDE.md` + `supervisor.md` + project/task/issue/conversation/worker 各 scope，ancestor-walk 优先级）存放经验、教训、技能、原则。业务实体（Project/Task/Plan/Issue）ground truth 在 Center，不在本地镜像 |
...
│
├── memory/                          # Agent 持久记忆（git 仓库；scoped 多文件，见 §9）
│   ├── .git/
│   ├── CLAUDE.md                    # global scope：跨 scope 的经验/教训/技能/原则
│   ├── supervisor.md                # supervisor 自我记忆（显式加载，不参与 ancestor walk）
│   ├── projects/{project_id}/CLAUDE.md
│   │   └── tasks/{task_id}/CLAUDE.md        # task scope
│   │   └── issues/{issue_id}/CLAUDE.md      # issue scope
│   ├── conversations/{conversation_id}/CLAUDE.md
...
`memory/` 是 Agent 的**持久记忆**——存放经验、教训、技能、原则。它是 **scoped 多文件**模型：记忆按 scope 分层存放，Agent 执行某 scope 工作时按 ancestor-walk 组装相关层级。

它与 `tasks/`、`plans/` 的职责边界清晰：

| 目录 | 存放内容 | Ground Truth |
|---|---|---|
| `memory/` | 经验、教训、技能、原则（scoped） | Agent 本地（git 管理） |
...
memory/
  .git/
  CLAUDE.md                                           # global scope（所有工作）
  supervisor.md                                       # supervisor 自我记忆（显式加载）
  projects/{project_id}/CLAUDE.md                     # project scope
  projects/{project_id}/tasks/{task_id}/CLAUDE.md     # task scope
  projects/{project_id}/issues/{issue_id}/CLAUDE.md   # issue scope
  conversations/{conversation_id}/CLAUDE.md           # conversation scope
  workers/{worker_id}/CLAUDE.md                       # worker scope
```

- 每个 scope 一个 `CLAUDE.md`（`supervisor.md` 例外，见下），git 管理版本历史，支持回溯与审计。
- **canonical 决策**：记忆模型 = scoped 多文件（不是单一 `MEMORY.md`）；ancestor-walk 优先级 = 越具体越优先。
- Reset memory 清除整个 `memory/` 目录（含 git 历史与所有 scope 文件）。

> **实现**：scoped 路径/校验/git 操作在 `internal/cognition/memory`（`path.go` / `gitops.go` / `skeleton.go`），运行时门面在 `runtime.go`（`Engine`）。

### 9.3 启动初始化（EnsureRootInit）

Agent CLI 进程启动时（`worker agent-supervisor` 子命令，`internal/cli/handlers_agentsupervisor.go`）对 `<home>/memory` 执行 `Engine.EnsureRootInit`：

1. `git init`（若尚未是 git 仓，幂等）。
2. seed global `CLAUDE.md` + `supervisor.md` skeleton（已存在则跳过）。

Best-effort：memory 初始化失败只记录日志、不阻断 claude 启动（memory 是增强项，非生存核心）。

### 9.4 读路径：ancestor-walk 与 prompt 注入

**ancestor 链**（broad → narrow，`AncestorScopes`）：

| scope | 链（broad → narrow） |
|---|---|
| task | global → project → task |
| issue | global → project → issue |
| conversation | global → conversation |
| worker | global → worker |
| project | global → project |

`supervisor.md` 不参与 ancestor-walk（cognition/02 §1）——它通过 `includeSupervisor` 标志显式追加。

**优先级**：越具体的 scope 优先级越高；组装进 prompt 时按 broad→narrow 顺序排列，最具体的指导最后出现并覆盖通用指导。缺失的 scope 文件被跳过；整条链为空时返回空串（调用方不注入）。

**注入点（对齐 harness/agentsupervisor）**：Agent CLI 启动时把 global + supervisor 记忆经 `Engine.HarnessContext` 组装成 memory block，随 work-queue operating instructions 一起进入 `--append-system-prompt`（`claudestream.BuildStreamingArgv` 的 `extraSystemPrompt`）——与 harness 同一通道，每次启动幂等重放、不进对话历史。该 block 同时给出 scoped 目录布局指南，使 Agent 在执行具体 scope 工作时自行查阅/写回对应 `CLAUDE.md`。

### 9.5 写路径与同步

**写入方**：Agent CLI（在工作期间自主归纳，直接用文件工具编辑对应 scope 的 `CLAUDE.md`；程序化写入用 `Engine.WriteScoped`）。

**同步流程（W2：仅本地 commit）**：
1. Agent 编辑 `memory/.../CLAUDE.md`。
2. supervisor 后台 memory-sync 线程定期（默认 30s）`Engine.CommitDirty`：脏树则 `git add -A + git commit`，干净树 no-op；进程退出前再做一次 final flush。
3. commit author = Agent 的 display 身份（`<agent-id>@agent-center`），历史归因到 Agent。

**W2 边界**：本轮只本地 commit。`git push` 到 Center Memory 远端（接收端）属 W1，本轮不接。

**一致性保证**：
- 未 commit 的修改是最新写入（进程内/文件系统可见）。
- 已 commit 的版本是持久化版本（进程崩溃可恢复）。

### 9.6 路径安全

- 仅允许在 `memory/` 目录内操作。
- 路径 containment guard 两层：
  - **词法层**（`AbsPath` / `validatePathComponent`）：拒绝 `../`、路径分隔符、空字节、保留名等会逃逸 `memory/` 的 scope 组件。
  - **运行时 symlink 层**（`guardSymlinkEscape`）：解析路径最深的已存在祖先，若经 symlink 解析后落在 `memory/` 之外则拒绝（读/写同样拒绝，返回 `ErrMemoryPathEscapes`）——词法层无法捕获的 symlink 逃逸由此兜住。
...
| P15 | Memory scoped 多文件 git commit 正常产生历史（W2 本地）；push 到 Center 远端属 W1 | git 操作验证；ancestor-walk 读 + scoped 写 commit + containment（含 symlink）见 §9 |
