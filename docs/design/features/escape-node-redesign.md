...
> **状态**：设计稿（已与 oopslink 对齐，采纳**最优解**：**A 全做；C 只保留 `escalate`，`abandon`/`force-integrate` 列为 deferred / demand-gated**）。
...
> **采纳的最优解（重要）**：经复盘，**A（escape 降为 Decision 终态 + 升级）是修复「逃生节点不优雅」的最小且充分解，全做**。**C（穷尽处置策略化）收敛为：`escalate` 是唯一的自动处置（即 A 的行为）；`abandon` 与 `force-integrate` 改为「等真实 cycle 提出需求再做」的 deferred 增量**——它们引入的 Gate degraded 语义手术 / `pass_forced` 新 outcome 属投机性通用化，当前无需落地。穷尽罕见且 PD 本就在环里，临场人裁「继续返工 / 手动放弃 / 强合」即可覆盖。下文 §3 保留 `abandon`/`force-integrate` 的完整设计**仅作 future 参考**，不进本轮实施范围（见 §6）。
...
> **实施范围（最优解收敛）**：本轮**只实现 `escalate`**（= A 行为，§3.1），它已是默认且唯一自动处置。`abandon`（§3.2）与 `force-integrate`（§3.3）**列为 deferred**——完整设计保留于下，仅当某真实 cycle 提出「自动放弃 feature 让 Gate 降级放行」或「强制集成未过评审的 feature」需求时再启动。届时无需重做 A，按下文增量接入即可。

把「回环耗尽后怎么办」做成 **per-feature 策略**，scaffold 时选定，引擎按策略分发（**本轮仅 `escalate` 落地，其余 deferred**）：
...
| **F2**（**deferred**）policy 字段 + `abandon`（含 Gate 降级） | `ExhaustionPolicy` 枚举 + schema 列 + Task/CycleNodeMeta 字段 + scaffold 入参；abandon 的 Gate seq 上游 `Done‖Skipped` 放宽（仅 `Role==gate`）+ `PlanView.degraded` | **不进本轮**——仅当真实 cycle 需要「自动放弃 feature 让其余 ship」时启动。届时：单测 scaffold 写 policy；abandon 下 Gate 在「被 abandon 的 feature skipped + 其它 done」时 ready、`degraded==true`、只对 gate 放宽；deployed e2e 两 feature 一 abandon 一 pass 走到 Ship |
| **F3**（**deferred**）`force-integrate` | `pass_forced` outcome + Integrate 条件边接受多值 When + force 标记 + 通知；仍要求 gate 绿、不绕 merge guard | **不进本轮**——仅当真实需求出现再做。届时验收同上文 §3.3 |

> **本轮 PR 顺序：F1 + F4 一起上**（F4 是 F1 的配套文档/工具描述清理，无独立代码风险）。**F1 即「逃生节点不优雅」的完整修复**——删死顶点、删 plan_view 特判、escalate 升级。**F2/F3 deferred**，需求驱动时再增量叠加（设计已就绪，不重做 A）。
