...
<agent_root>/executors/<executor_id>/
  ...
  progress.jsonl  # executor append 流式进度（监工 tail 转播 chat）—— 采样流，非审计级，见 §2.1
...
### 2.1 `progress.jsonl` 是采样流，**不是审计日志**（I109 ②）

> **它能证明某件事发生过；它永远不能证明某件事没发生过。**

append 是 best-effort（写失败被吞——转播失败不该杀死 run）、散文活动按 ~15s 心跳**采样**、
`message` 是渲染后被裁剪的短note、run 可能在动作与 append 之间死掉。**因此"没有记录"不是
"没有发生"的证据。**

这条不是样板话。真实事故：`grep -c "push" progress.jsonl` = **0**，据此几乎把一个诚实的
executor 定性为「谎报交付」，且污染证据链已同步给第三方；决定性反证是
`git reflog show --date=iso refs/remotes/origin/<branch>` → `update by push @23:49:24`，比
`state=done` 早 22 秒 —— push 真实发生，那个 0 是采样 + 裁剪的产物。同一份日志里
`git add`/`commit`/`rebase` 命中，仅仅因为它们恰好落在裁剪点之前。

**要下否定断言，请走记录源**：git reflog / 远端 / 中心的 task·delivery 行。

I109 ② **收窄**（未消除）这个缺口：每个 **tool 调用**各记一条（不再挤心跳），且
`ProgressEntry.Tools`（`["Bash","git","push"]`）在 `message` 被裁剪后仍保留"跑了什么"。
更完整了，仍不是审计级。
