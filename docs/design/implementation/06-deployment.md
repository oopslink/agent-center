> 📌 **v2 update applied (P12 S6, 2026-05-24)** — v2 撤回了 Bridge BC + 飞书集成 (per [ADR-0031](../decisions/0031-v2-drop-bridge-vendor-integration.md))；vendor 凭据安装 / 端口出站 / EnvironmentFile / feishu.env 等步骤已删；Web Console loopback bind 替代 vendor 入口（[ADR-0037](../decisions/0037-web-console-as-main-user-ui.md)）；master.key (ADR-0026) 取代 vendor secret file。
...
└── master.key                         # SecretManagement 主密钥（mode 0600；ADR-0026）
...
| /etc/agent-center | 配置 + master.key（ADR-0026） | KB | 配置改动时（手动 git）|
...
# 3b. 生成 SecretManagement master key (ADR-0026)
sudo install -m 0600 -o agent-center -g agent-center /dev/null \
  /etc/agent-center/master.key
sudo head -c 32 /dev/urandom | sudo tee /etc/agent-center/master.key > /dev/null
