...
└── feishu.env                         # 凭据 env 文件（systemd EnvironmentFile）
...
EnvironmentFile=-/etc/agent-center/feishu.env
Restart=on-failure
RestartSec=5s
...
| /etc/agent-center | 配置 + feishu.env | KB | 配置改动时（手动 git）|
...
| 事件 | 触发条件 | 路由 |
|---|---|---|
| `worker.offline { reason, message }` | 心跳超时 / 主动 disconnect | Bridge 推飞书 + Web Console |
...
| 443/tcp | HTTPS（飞书 WebSocket）| 出站 | open.feishu.cn |
| 22/tcp | SSH | 入站 | 运维人员（白名单 IP） |
| 其它 | - | 关 | 所有 |

**无 HTTP webhook / 无入站飞书连接** —— Center 主动出站连飞书 WebSocket，不需要域名 / TLS cert / Reverse proxy。
...
# 复制 04-configuration § 8.1 范例 + 改 feishu.app_id 等
YAML
...
Worker enroll + Feishu setup 都做完 → 第一个 task 可以派单。
