...
DDD 战略设计层。具体聚合 / 实体 / 值对象的字段与状态机迁移细节，分散在后续各章节（[task-runtime/00-overview.md](../../retired/task-runtime/00-overview.md) / [discussion/00-overview.md](../../retired/discussion/00-overview.md) / 等）。

> 🆕 **v2 状态**（per [ADR-0031 Drop Bridge](../../decisions/0031-v2-drop-bridge-vendor-integration.md) + [ADR-0039 Conversation 业务模型 v2 统一](../../decisions/0039-conversation-business-model-v2-unified.md) + 2026-05-23 删干净）：
> - BC7 **Bridge BC 完全删除**；任何 vendor 接入（飞书 / DingTalk / Web chat / etc.）+ 相关概念（FeishuBridge / LarkCard / ChannelBinding / vendor_msg_ref / vendor route）**全删**
> - BC8 **SecretManagement** 新增（user secrets 中心化管理，[ADR-0026](../../decisions/0026-user-secret-management-bc.md)）
> - v2 用户主入口 = Web Console ([ADR-0037](../../decisions/0037-web-console-as-main-user-ui.md)) + CLI ([ADR-0038](../../decisions/0038-cli-ux-enhancement.md))
> - v3+ 重新设计 vendor 接入（届时无 v1 包袱）
>
> 本文件下面 § 内若有零星 vendor 描述属遗漏；以本横幅为准。
...
| 术语 | 上下文 | 定义 |
|---|---|---|
| **Task（任务）** | TaskRuntime | 工作单元身份；独立 Aggregate Root，**身份不变**。属于一个 project；可被多次 dispatch（每次产生一条 TaskExecution）。**只有 center 能创建 task，无野任务**（[conventions § 1](../../../rules/conventions.md#-1-单一来源--无野任务)）。**跟 `kind=task` Conversation 1:1 绑定**（`task.conversation_id` 字段；按来源分同步建 / 懒创建两支，见 [ADR-0017](../../decisions/0017-task-as-conversation.md)）。详见 [task-runtime/01-task.md](../../retired/task-runtime/01-task.md) |
| **TaskExecution（任务执行）** | TaskRuntime | Task 的一次执行（dispatch → 结束的运行痕迹）。**独立 Aggregate Root**（持 `task_id` 强引用 Task；[ADR-0019](../../decisions/0019-bc-scheduling-execution-merged-to-task-runtime.md)）；1:N 于 Task；retry = 同 task 新 TaskExecution，**不创建新 task**。每条 TaskExecution 永远绑一个 worker。A2A 状态机驱动。详见 [task-runtime/02-task-execution.md](../../retired/task-runtime/02-task-execution.md) 与 [ADR-0010](../../decisions/0010-task-execution-two-layer-model.md) |
| **InputRequest（请示）** | TaskRuntime | TaskExecution 进行中 agent 卡住需要外部输入的**同步阻塞**请求 |
| **Issue（议题）** | Discussion | 待讨论的事。可由 user / supervisor / worker (agent) 提起；多轮讨论 conclude 后可能产生 0/1/N 个 Task。**跟 `kind=issue` Conversation 1:1 绑定**（`issue.conversation_id` 字段；按来源分同步建 / 懒创建两支，见 [ADR-0021](../../decisions/0021-issue-as-conversation.md)）。详见 [discussion/00-overview.md](../../retired/discussion/00-overview.md) |
...
| **AgentInstance（agent 实例）** | Workforce | 一等公民身份：用户起名（如 `coder-mbp`）+ 绑定 Worker + 配置（instructions / mcp / skill）+ 状态机（idle / active / sleeping / archived）。**实际执行进程**仍是 per-TaskExecution spawn 的 CLI 子进程，但所有 execution 都"记账"到对应 AgentInstance；1:N（一个 AgentInstance 可同时承担多个并行 TaskExecution，受 Worker concurrency + AgentInstance.max_concurrent cap）。**Supervisor 是 built-in AgentInstance**（`is_builtin=true`，`worker_id=NULL`，跑在 center 机；archive 禁止；[ADR-0029](../../decisions/0029-supervisor-as-builtin-agent-instance.md)）。详见 [workforce/04-agent-instance.md](../tactical/workforce/04-agent-instance.md) 与 [ADR-0024](../../decisions/0024-agent-instance-first-class.md) |
| **UserSecret（用户密钥）** | SecretManagement | 用户管理的密钥实体：name（全局唯一）+ kind（mcp / cloud_credential / ...）+ AES-GCM 加密 value + state machine（active / revoked）。仅服务于 user-domain（MCP env vars 等）；系统内部凭证（BootstrapToken / session_token）不归本实体。详见 [secret-management/01-user-secret.md](../tactical/secret-management/01-user-secret.md) 与 [ADR-0026](../../decisions/0026-user-secret-management-bc.md) |
...
| **WorkerProjectProposal（提议）** | Workforce | Worker 自动扫描发现的"候选项目映射"。需要用户在 Web Console / CLI 确认才能升级成 WorkerProjectMapping。状态机 pending → accepted/ignored/superseded |
| **Artifact（产物）** | TaskRuntime | TaskExecution 产生的文件 / 引用（PR URL / 文件 / 报告 / 等），独立表 `artifacts`，归属 execution（TaskExecution 的子实体），free-form kind。详见 [task-runtime/02-task-execution.md § 12](../../retired/task-runtime/02-task-execution.md) |
...
> ⚠️ **RETIRED BCs (v2.7 #131 carve-out)**: **BC1 TaskRuntime** + **BC2 Discussion** 已从代码库删除（code + tables removed），不再是 live BC。它们的职责已迁移到 **ProjectManager BC（pm.Task / pm.Issue）+ Agent work-items**。下文 BC1 / BC2 小节作为历史设计记录保留；其战术设计已移至 [retired/task-runtime/](../../retired/task-runtime/00-overview.md) + [retired/discussion/](../../retired/discussion/00-overview.md)。当前 v2.7 设计见 sites/designs/v2.7/。

### ~~BC1: TaskRuntime（任务运行时）~~ — RETIRED (v2.7 #131 — replaced by ProjectManager pm.Task/pm.Issue + Agent work-items)
...
### ~~BC2: Discussion（讨论）~~ — RETIRED (v2.7 #131 — replaced by ProjectManager pm.Issue + Agent work-items)
...
**核心聚合**:
- `Worker`（根）+ `BootstrapToken`（实体，子从属；enroll 凭证 stateful，[ADR-0023](../../decisions/0023-worker-enroll-lightweight.md)）+ `WorkerProjectMapping`（实体，子从属；已生效的稳定映射）
- `AgentInstance`（根，独立聚合；agent 一等公民身份，[ADR-0024](../../decisions/0024-agent-instance-first-class.md)）
- `WorkerProjectProposal`（根，独立聚合；自动发现的候选，等用户 Web Console / CLI 确认才升级为 Mapping）
- `Project`（根，独立聚合）
...
**职责**: 系统**内部**的会话消息时间线存储。纯业务模型（v2 vendor 撤回 per [ADR-0031](../../decisions/0031-v2-drop-bridge-vendor-integration.md)）；承载所有领域 thread（用户对话 / Task / Issue / Channel 话题群 / 通知）的消息时间线 (per [ADR-0039](../../decisions/0039-conversation-business-model-v2-unified.md))。

**核心聚合**:
- `Conversation`（根） + `Message`（实体，从属于 Conversation）
- `Identity`（根，独立聚合）—— 3 kinds (user / agent / system)；ID 格式 `kind:id` (per [ADR-0033](../../decisions/0033-identity-model-refactor.md))
...
**核心操作**:
- `conversation send <conv> <content>` (**内部写入**；Web Console / CLI 用户主入口)
...
```mermaid
flowchart TB
    subgraph Entry [v2 用户入口]
        WEB[Web Console<br/>SPA React + Go API + SSE<br/>ADR-0037]
        CLI[CLI<br/>ADR-0038]
    end

    subgraph Domain [领域层 BC1-BC4 + BC6 + BC8]
        BC1["BC1 TaskRuntime<br/>Task / TaskExecution / InputRequest / Artifact<br/>协议 dispatch / kill / reconcile / timeout<br/>+ worker 侧运行时 shim / workspace / JSONL"]
        BC2["BC2 Discussion<br/>Issue 单聚合"]
        BC6["BC6 Conversation<br/>Conversation / Message / Identity<br/>消息时间线 (kind=channel/dm/task/issue/adhoc/notification)<br/>ADR-0039"]
        BC3["BC3 Workforce<br/>Worker / AgentInstance / Project / Mapping / Proposal<br/>算力节点 + agent 一等公民"]
        BC8["BC8 SecretManagement<br/>UserSecret AR + AES-GCM 加密<br/>ADR-0026"]
    end

    BC2 <-.Shared Kernel 1:1<br/>issue.conversation_id.-> BC6
    BC1 <-.Shared Kernel 1:1<br/>task.conversation_id.-> BC6
    BC2 -- Customer-Supplier<br/>IssueConcludeSpawn --> BC1
    BC1 -- Customer-Supplier<br/>open-issue --> BC2
    BC1 <-.Shared Kernel<br/>worker_id / project_id / agent_instance_id.-> BC3
    BC2 <-.Shared Kernel<br/>project_id.-> BC3
    BC3 -- Customer-Supplier<br/>SecretRef resolve --> BC8

    WEB == HTTP API ==> BC1
    WEB == HTTP API ==> BC2
    WEB == HTTP API ==> BC3
    WEB == HTTP API ==> BC6
    CLI == application service ==> BC1
    CLI == application service ==> BC2
    CLI == application service ==> BC3
    CLI == application service ==> BC6
    CLI == application service ==> BC8

    subgraph CrossCutting [跨切 actor]
        BC4["BC4 Cognition Supervisor<br/>SupervisorInvocation / Memory / DecisionRecord<br/>事件驱动 spawn 子进程做决策"]
    ...
    BC4 ==>|"User via tools<br/>dispatch / kill / issue conclude<br/>conversation send"| BC1
    ...
    class BC1,BC2,BC3,BC6,BC8 domainBox
    class WEB,CLI entryBox
    class BC5 obsBox
    class BC4 cogBox
```
...
- **领域层 BC1-BC4 / BC6 / BC8**：零外部 vendor 依赖；只跟其它 BC 通过 Shared Kernel / Customer-Supplier 模式交互；所有外发通过 emit domain events
- **用户入口面（Web Console + CLI）**：v2 唯一用户接入面；都过 application service 层；兄弟前端
- **Observability BC5（Open Host / Subscribe-only）**：所有 BC emit 事件到 `events` 表；只订阅不发起；提供统一查询接口（inspect / query / ps / stats / logs）
- **Cognition BC4（跨切）**：Supervisor 通过 CLI 工具调任何 BC 的动作命令（同 user 用同一套）；不为 supervisor 单造 RPC
- **~~Bridge BC7~~**：v2 已撤回（per ADR-0031）；v3+ 重新设计
...
| TaskRuntime ↔ Workforce | Shared Kernel | Task / TaskExecution 引用 worker_id / project_id / agent_instance_id（[ADR-0024](../../decisions/0024-agent-instance-first-class.md)）|
| Workforce → SecretManagement | Customer-Supplier | AgentInstance.config.mcp_config 内嵌 SecretRef；worker daemon spawn 前调 SecretResolutionService.resolve 拿明文（[ADR-0026](../../decisions/0026-user-secret-management-bc.md) / [ADR-0027](../../decisions/0027-mcp-per-agent-injection.md)）|
...
| Discussion ↔ Conversation | Shared Kernel / 1:1 | `issue.conversation_id` 强引用 `kind=issue` Conversation（1:1）；`issue.opened` + `conversation.opened` 同事务（[ADR-0039](../../decisions/0039-conversation-business-model-v2-unified.md)）；议事消息走 Conversation Message |
| TaskRuntime ↔ Conversation | Shared Kernel | `task.conversation_id` 强引用 `kind=task` Conversation（1:1）；`task.created` + `conversation.opened` 同事务；worker daemon / InputRequest 写 Message 走 Conversation API（[ADR-0039](../../decisions/0039-conversation-business-model-v2-unified.md)）|
| Cognition → ALL | "User" via tools | Supervisor 通过 CLI 工具调用其它上下文（`dispatch` / `query` / `issue comment` / `conversation send` / 等）—— 都是内部写入 |
