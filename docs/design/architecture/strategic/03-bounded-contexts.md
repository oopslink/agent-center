...
> - **CodeRepo BC 新增**：工作空间级代码仓库实体（org-scoped），含加密凭据。代码包 `internal/coderepo`（含 `provider/` 子包）
> - **Conversation BC 扩展**：kind 体系扩充为 5 种（dm / channel / task / issue / plan）；Channel 为 kind 而非独立聚合；新增 DM 去重、FollowState、ReadState、OwnerContext
> - **Workforce BC 精简**：`WorkerProjectProposal` / `WorkerProjectMapping` 已 RETIRED（v2.7 #131），新增 `BootstrapToken` 注册令牌聚合
> - BC1 **TaskRuntime** + BC2 **Discussion** + BC7 **Bridge** 已 RETIRED（历史设计见 [retired/](../../retired/)）
> - **Supervisor** 暂停主动开发（ADR-0044），代码骨架保留；SupervisorInvocation / DecisionRecord 仅 DB 表，无 Go 领域模型
...
| **BootstrapToken（注册令牌）** | Workforce | Worker 注册时使用的一次性令牌。状态机 active / used / revoked。代码包 `internal/workforce`（`bootstrap_token.go`） |
...
| **CodeRepo（代码仓库实体）** | CodeRepo | 工作空间级（org-scoped）代码仓库实体：label / url / provider / default branch + 加密凭据。与 ProjectManager 的 `CodeRepoRef`（项目→仓库引用关系）不同，本实体是**仓库本体**。代码包 `internal/coderepo` |
| **Conversation（会话）** | Conversation | 系统**内部**的消息时间线存储；纯业务模型，无 vendor 依赖；5 种 kind：`dm`（1:1 私信）/ `channel`（频道）/ `task`（跟 Task 1:1）/ `issue`（跟 Issue 1:1）/ `plan`（跟 Plan 1:1）（详 [ADR-0039](../../decisions/0039-conversation-business-model-v2-unified.md)） |
...
> Task **没有 `failed`**。失败是某次执行的状态，不是 task 的状态。详见 [ADR-0010](../../decisions/0010-task-execution-two-layer-model.md)。
...
agent-center 的领域划分为 **11 个 live 限界上下文**。Web Console / CLI / BlobStore 不是 BC（属于表现层 / 基础设施）。已 retired 的 BC（TaskRuntime / Discussion / Bridge）的历史战术设计见 [retired/](../../retired/)。
...
**职责**: Worker 注册 / 在线状态 / BootstrapToken（注册令牌）/ 心跳 / 能力上报 / Worker 配置

> **注意**：v2.7 AgentInstance 已独立为 Agent BC（`internal/agent`）；Project 全生命周期管理已迁至 ProjectManager BC（`internal/projectmanager`）。本 BC 仅保留 Worker 节点管理能力。
>
> ⚠️ `WorkerProjectProposal` / `WorkerProjectMapping` 已 RETIRED（v2.7 #131，对应 DB 表已删除，项目映射职责迁至 ProjectManager BC）。

**代码包**: `internal/workforce`（含 `service/` / `sqlite/` 子包）

**核心聚合**:
- `Worker`（根；`worker.go`）
- `BootstrapToken`（根，独立聚合；`bootstrap_token.go`；注册令牌，状态机 active / used / revoked）

**核心事件**:
- `worker.enrolled` / `worker.online` / `worker.offline` / `worker.heartbeat` / `worker.config.updated`
- `bootstrap_token.created` / `bootstrap_token.used` / `bootstrap_token.revoked`

**核心操作**: `worker join` / `worker list / status` / bootstrap token create / revoke
...
**核心聚合**:
- `SupervisorInvocation`（根，DB 表 `supervisor_invocations`；**仅 DB schema + migration，当前无 Go 领域模型文件**） + `DecisionRecord`（实体，从属，append-only；同样仅 DB 层）
...
**代码包**: `internal/observability`（含 `escalator/` / `peek/` / `query/` / `sqlite/` 子包）
...
**职责**: 系统**内部**的会话消息时间线存储。纯业务模型；承载所有领域 thread 的消息时间线。子包 `replyguard/`（回复防重）、`service/`（应用服务）、`sqlite/`（持久化）。

> **注意**：v2.6 起 Identity 已独立为 Identity BC（ADR-0040，`internal/identity`）。本 BC 不再管理 Identity。

**代码包**: `internal/conversation`（含 `replyguard/` / `service/` / `sqlite/` 子包）

**Conversation Kind（5 种）**:
- `dm`：1:1 私信（DM 去重键 `dm.go` 保证同一参与者对只有一个 DM）
- `channel`：频道（需 name；`service/channel_management.go` 管理创建 / 归档）
- `task`：跟 Task 1:1
- `issue`：跟 Issue 1:1
- `plan`：跟 Plan 1:1

**核心聚合**:
- `Conversation`（根；`conversation.go`） + `Message`（实体，从属于 Conversation；`message.go`）
- `OwnerContext`（值对象；Conversation 归属上下文，`owner_context.go`）
- `FollowState`（实体；用户关注状态，`follow_state.go`）
- `ReadState`（实体；已读状态，`read_state.go`）

> Channel 不是独立聚合根——它是 `ConversationKind = "channel"` 的 Conversation，管理逻辑在 `service/channel_management.go`。
...
- Channel 管理：create / archive（`service/channel_management.go`）
- DM 管理：自动去重创建（`dm.go` DMKey + DB partial unique index）
...
### BC-CodeRepo: CodeRepo（代码仓库）

**职责**: 工作空间级（org-scoped）代码仓库实体管理——仓库元数据（label / url / provider / default branch）+ 加密凭据（AES-GCM，复用 secretmgmt master key）+ CRUD。**不是 VCS 集成**——provider viewing（commits / branches）和 agent MCP 面是后续阶段。

> **注意**：与 ProjectManager BC 内的 `CodeRepoRef` 是不同概念——`CodeRepoRef` 是 Project 子实体，存项目到仓库的**引用关系**；`CodeRepo.Repo` 是**仓库本体**（url / credential），org-scoped 独立存在。

**代码包**: `internal/coderepo`（含 `provider/` / `service/` / `sqlite/` 子包）

**核心聚合**:
- `Repo`（根，独立聚合；`repo.go`；含加密凭据 ciphertext + nonce，明文不存聚合）

**核心能力**:
- `provider/`：仓库托管商适配（github / gitlab / git）
- CRUD + 凭据加密存储 / 解密读取

**关键约束**:
- 明文凭据不存聚合、不入 event、API 返回时 mask
- 加密复用 secretmgmt 的 master key（AES-GCM）
...
flowchart TB
    ...
    subgraph Essential [Supporting-Essential 领域层]
        ...
        WF["Workforce<br/>Worker / BootstrapToken"]
        CONV["Conversation<br/>5 kinds: dm / channel /<br/>task / issue / plan<br/>replyguard"]
    ...
    subgraph Peripheral [Supporting-Peripheral 领域层]
        ...
        CR["CodeRepo<br/>Repo (org-scoped) /<br/>加密凭据 / provider"]
    ...
    class SEC,CR,FILES peripheralBox
...
- **Supporting-Peripheral（SecretManagement / CodeRepo / Files / Observability）**：降级可运行
...
| CodeRepo → SecretManagement | Customer-Supplier | CodeRepo 加密凭据复用 secretmgmt 的 master key（AES-GCM） |
...
| **AdminToken** | admin API 令牌管理（entity + service + sqlite） | `internal/admintoken` |
| **AutoAssign** | 项目级自动分配开关配置 | `internal/autoassign` |
| **Concurrency** | per-agent executor 并发视图共享类型（心跳上报 + API 展示） | `internal/concurrency` |
| **RuntimeFS** | agent runtime file browser 协议 + 请求/响应关联器 | `internal/runtimefs` |
...
| **Cognition** | `cognition`（`memory/` / `reminder/` / `wakeguard/`） | `supervisor.*` | Memory 走 file-based git 仓不入 DB；SupervisorInvocation 仅 DB 表，无 Go 领域模型 |
...
| **Workforce** | `workforce`（`service/` / `sqlite/`） | `worker.*` / `bootstrap_token.*` | Worker 节点管理 + BootstrapToken 注册令牌；WorkerProjectProposal / Mapping 已 retired |
| **Conversation** | `conversation`（`replyguard/` / `service/` / `sqlite/`） | `conversation.*` / `message.*` | 5 种 kind：dm / channel / task / issue / plan |
| **Environment** | `environment`（`controlstream/` / `service/` / `sqlite/`） | `environment.*` | Worker 环境管理 |
| **Observability** | `observability`（`escalator/` / `peek/` / `query/` / `sqlite/`） | (不产事件) | 跨上下文事件总线 |
| **SecretManagement** | `secretmgmt`（`service/` / `sqlite/`） | `user_secret.*` | 用户密钥管理 |
| **CodeRepo** | `coderepo`（`provider/` / `service/` / `sqlite/`） | `code_repo.*` | 工作空间级代码仓库实体（org-scoped），含加密凭据 |
...
| 层 | 内容 | 权威定义位置 |
|---|---|---|
| **领域事件流** | `events` 表 schema + 各 BC emit 的 event_type 闭集（`project.*` / `issue.*` / `task.*` / `plan.*` / `worker.*` / `bootstrap_token.*` / `agent_instance.*` / `supervisor.*` / `conversation.*` / `identity.*` / `environment.*` / `file.*` / `user_secret.*` / `code_repo.*`）+ payload 形态（含 reason+message 双字段，[§ 16](../../../rules/conventions.md#-16-错误--状态信息双字段reason--message)）| 各 BC 战术文档 / `tactical/observability/00-overview.md` |
