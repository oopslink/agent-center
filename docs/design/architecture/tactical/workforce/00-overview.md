...
> 🗑️ **TaskRuntime / Discussion BC 已退役（v2.7 #131 carve-out）。** 下文对 TaskRuntime / Discussion 的引用（**Workforce ↔ TaskRuntime / Discussion Shared Kernel**、dispatch / TimeoutScanner / DispatchService / `worker.offline` 处理 active execution 等）为**历史记录、非当前架构**：职责已由 pm（Project / Task / Issue）+ agent-work-item 模型承接（worker-offline → agent-unavailable 走 ADR-0050 on-read 派生）。链接指向 `docs/design/retired/`（历史留存）。当前架构见 sites/designs/v2.7/；按新模型重写见 task #144-b。
...
>
> **本 BC 不管"怎么干活"**：dispatch / shim / workspace 物理创建 / Agent CLI 子进程 / JSONL 解析 / Artifact / kill 进程级机制 / reconcile worker 端 → 全部归 [TaskRuntime BC](../../../retired/task-runtime/00-overview.md)（[ADR-0019](../../../decisions/0019-bc-scheduling-execution-merged-to-task-runtime.md) carve 后的边界）。
...
| 维度 | 内容 |
|---|---|
| **聚合管理** | Worker（AR + 子从属 BootstrapToken + WorkerProjectMapping）/ AgentInstance（独立 AR，[ADR-0024](../../../decisions/0024-agent-instance-first-class.md)）/ WorkerProjectProposal（独立 AR）/ Project（独立 AR）|
| **Worker 生命周期** | enroll token 签发 → `agent-center join` 兑换 session token / 心跳 / 在线状态 / heartbeat timeout（[ADR-0023](../../../decisions/0023-worker-enroll-lightweight.md)）|
| **AgentInstance 生命周期** | 用户 `agent create` → state machine `idle ↔ active → sleeping → archived`；跟 Worker 状态联动；持久 home_dir 沉淀 agent-level instructions（[ADR-0024](../../../decisions/0024-agent-instance-first-class.md)）|
| **行为配置主权** | concurrency / discovery / capabilities 在 center DB 主导；worker 端通过 reconcile 拉取 + `worker.config.updated` 长连推送同步 |
| **自动发现 + 用户确认** | Worker 周期扫 scan_paths（来自 center 下发的 WorkerConfig）找候选项目 → 写 Proposal → ~~飞书卡片~~ 用户 UI（v2：Web Console / CLI，per ADR-0031）→ 用户拍板 → 升级为 Mapping |
| **Project 元数据** | project_id / name / kind / default_agent_cli 等；自动从 accepted Proposal 创建 |
| **不持有的状态** | RETIRED / historical：Task / TaskExecution / per-execution 运行时（已 carve 到 TaskRuntime BC）|
...
- `AgentInstance`（聚合根，独立；agent 一等公民身份，绑到 worker，含 config + 状态机；[ADR-0024](../../../decisions/0024-agent-instance-first-class.md)）
...
- **Workforce ↔ TaskRuntime**：**RETIRED / historical Shared Kernel**（Task / TaskExecution 引用 `worker_id` + `project_id`；TaskExecution dispatch 时 center 查 WorkerProjectMapping 取 base_path）
- **Workforce ↔ Discussion**：**RETIRED / historical Shared Kernel**（Issue 引用 `project_id`）
- ~~**Bridge → Workforce**：Customer-Supplier（用户在飞书点 Proposal 卡片按钮 → Bridge 翻译为 `accept-proposal` / `ignore-proposal` API 调用）~~ (v2 撤回 per ADR-0031)
...
| **AgentInstance** | [04-agent-instance.md](04-agent-instance.md) | 4 态（idle / active / sleeping / archived）| ULID `id`；`name` 全局唯一；`worker_id` v2 不可变（[ADR-0024](../../../decisions/0024-agent-instance-first-class.md)）|
...
| 实体 | 从属 | 位置 |
|---|---|---|
| **BootstrapToken** | Worker（独立表 `bootstrap_tokens`） | [01-worker.md § 3](01-worker.md)（[ADR-0023](../../../decisions/0023-worker-enroll-lightweight.md)）|
...
| VO | 用在哪 | 描述 |
|---|---|---|
| **SessionToken** | Worker 长连接认证 | 长期；worker 端落 `~/.agent-center/credentials` (mode 0600)；session 失效需要重新 enroll（[ADR-0023](../../../decisions/0023-worker-enroll-lightweight.md)）|
| **WorkerConfig** | center → worker 同步 | `{ concurrency, discovery, capabilities_enabled }`；reconcile response 返回 + `worker.config.updated` 长连推送（[ADR-0023](../../../decisions/0023-worker-enroll-lightweight.md)）|
| **Capability** | Worker.capabilities 数组元素 | v2 形态：`{ agent_cli, detected, enabled, version?, supports_mcp, supports_skills, supports_session }`；探测来源 + 用户开关 + adapter feature 上报（[ADR-0023](../../../decisions/0023-worker-enroll-lightweight.md) + [ADR-0030](../../../decisions/0030-agentadapter-matrix-expansion.md)）|
| **AgentInstanceConfig** | AgentInstance.config 字段（JSON）| `{ instructions_ref?, mcp_config?, skills?, ... }`；G1 约定 instructions；G4 约定 mcp_config（MCP 标准 schema + SecretRef，[ADR-0027](../../../decisions/0027-mcp-per-agent-injection.md)）；G5 约定 skills（后续）|
...
5. **AgentInstance.state 跟 Worker.status 联动**：worker → offline 时该 worker 上所有 idle/active 的 AgentInstance → sleeping；worker → online 时 sleeping 的 → idle（[ADR-0024](../../../decisions/0024-agent-instance-first-class.md)）
...
### 3.4 AgentInstanceManagementService（[ADR-0024](../../../decisions/0024-agent-instance-first-class.md)）
...
### 3.5 AgentInstanceLifecycleService（[ADR-0024](../../../decisions/0024-agent-instance-first-class.md)）
...
**职责**：用户在 Web Console / CLI 对 Proposal 做 accept / ignore / accept-with-edit 决策；同事务 spawn Project（如不存在）+ Mapping。（v2 撤回飞书 per ADR-0031）
...
| 命名冲突 | accept 时 suggested_project_id 已存在 → ~~飞书卡片标红~~ 用户 UI 提示让用户改（v2 per ADR-0031）|
...
| Center 行为 | mapping.status = invalidated（不删，保留血缘）；~~飞书提示~~ 用户 UI 提示用户决定是否重新映射（v2 per ADR-0031）；不自动迁移 |
...
### 4.3 AgentInstanceFactory（[ADR-0024](../../../decisions/0024-agent-instance-first-class.md)）
...
### 5.2 AgentInstanceRepository（[ADR-0024](../../../decisions/0024-agent-instance-first-class.md)）
...
### 5.3 BootstrapTokenRepository（sub-repo of Worker, [ADR-0023](../../../decisions/0023-worker-enroll-lightweight.md)）
...
- AgentInstance 是独立 AR，通过 worker_id 弱关联到 Worker（[ADR-0024](../../../decisions/0024-agent-instance-first-class.md)）
...
- BootstrapToken reissue 同事务 revoke 旧 active + insert 新 active（[ADR-0023](../../../decisions/0023-worker-enroll-lightweight.md)）：跨同一 Repository 的两次写，行锁防并发
- AgentInstance state 跟 Worker.status 联动通过 `BulkUpdateStateByWorker` 批量切换（[ADR-0024](../../../decisions/0024-agent-instance-first-class.md)）
...
| **AgentInstance → Worker**（`agent_instance.worker_id`） | 强 / v2 不可变 | tx 同步（create 时填）；删 Worker 前置要求所有 agent archived | AgentInstanceManagementService create | [ADR-0024](../../../decisions/0024-agent-instance-first-class.md) |
| **TaskRuntime ← Workforce**（RETIRED / historical：`task.project_id` / `task_execution.worker_id` / `task_execution.project_id` / `task_execution.agent_instance_id`）| 强 / 不可变 | tx 同步（TaskFactory 创建时填） | Task / TaskExecution 创建 | [ADR-0024](../../../decisions/0024-agent-instance-first-class.md) |
| **Discussion ← Workforce**（RETIRED / historical：`issue.project_id`） | 强 / 不可变 | tx 同步 | Issue 创建 | - |
...
| `worker.offline` | RETIRED / historical：处理 worker_lost 的 active execution（详见 [TaskRuntime § 3.3](../../../retired/task-runtime/00-overview.md) TimeoutScanner）|
...
| `worker_project_proposal.proposed` | ~~推飞书卡片~~ 推用户 UI（Web Console / CLI，v2 per ADR-0031）让用户决策 accept / ignore / 改后 accept |
| `worker_project_mapping.invalidated` | ~~飞书提示~~ 用户 UI 提示（v2 per ADR-0031）用户决定是否重新映射 |
| `project.created` / `project.removed` | 一般不直接动作（除非有等候该 project 的 open task）|

详见 [cognition/00-overview.md](../cognition/00-overview.md)。

### 7.2 ~~Bridge 渲染（outbound）~~ (v2 撤回 per ADR-0031)
...
| Workforce ↔ TaskRuntime | RETIRED / historical Shared Kernel | Task / TaskExecution 引用 worker_id / project_id；TaskExecution dispatch 时 center 查 WorkerProjectMapping 取 base_path |
| Workforce ↔ Discussion | RETIRED / historical Shared Kernel | Issue 引用 project_id |
| ~~Bridge → Workforce~~ | ~~Customer-Supplier~~ | ~~inbound 时 Bridge 调 accept-proposal / ignore-proposal API~~ (v2 删 per ADR-0031) |
...
- [ADR-0023 Worker Enroll 轻量化（草案）](../../../decisions/0023-worker-enroll-lightweight.md)（一行命令接入 + BootstrapToken Entity + 服务端配置主导）
- [ADR-0024 AgentInstance 一等公民化（草案）](../../../decisions/0024-agent-instance-first-class.md)（agent 升级为独立 AR + 1:N 并发 + home_dir + state machine）
- [ADR-0026 SecretManagement BC（草案）](../../../decisions/0026-user-secret-management-bc.md)（AgentInstance.config.mcp_config 内嵌 SecretRef，secret 解析路径）
- [ADR-0027 MCP per-agent 注入（草案）](../../../decisions/0027-mcp-per-agent-injection.md)（mcp_config schema + 注入流程）
...
- [04-agent-instance.md](04-agent-instance.md) — AgentInstance 独立 AR（[ADR-0024](../../../decisions/0024-agent-instance-first-class.md)）

### 跨 BC 协作文档

- [task-runtime/00-overview.md](../../../retired/task-runtime/00-overview.md) — TaskRuntime BC 入口（含 ReconcileService / DispatchService 协议视图）
- [task-runtime/02-task-execution.md § 9-12](../../../retired/task-runtime/02-task-execution.md) — worker 端运行时（per-execution 实施细节）
- [discussion/00-overview.md](../../../retired/discussion/00-overview.md) — Issue 引用 project_id
- [conversation/00-overview.md](../conversation/00-overview.md) — Identity / ChannelBinding（用户跟 ~~vendor~~ 渠道的关联，跟 Worker 是不同维度）
