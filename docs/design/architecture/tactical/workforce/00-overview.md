...
>
> 🗑️ **AgentInstance AR 已退役（v2.7 #107 refactor）。** 下文对 AgentInstance 的引用（AR / ManagementService / LifecycleService / Factory / Repository / 状态联动等）为**历史记录、非当前架构**：Agent 身份 + 生命周期已由独立的 **Agent BC**（`internal/agent/`，ADR-0049）承接。历史设计文档见 [`docs/design/retired/workforce/04-agent-instance.md`](../../../retired/workforce/04-agent-instance.md)。
...
| 维度 | 内容 |
|---|---|
| **聚合管理** | Worker（AR + 子从属 BootstrapToken + WorkerProjectMapping）/ WorkerProjectProposal（独立 AR）/ Project（独立 AR）|
...
| **自动发现 + 用户确认** | Worker 周期扫 scan_paths（来自 center 下发的 WorkerConfig）找候选项目 → 写 Proposal → 用户 UI（Web Console / CLI）→ 用户拍板 → 升级为 Mapping |
...
- 行为动词：`Issue` / `Reissue` / `Revoke`（token 签发 / 重发 / 撤销）/ `Join`（worker 一行接入）/ `Enroll`（仓库 / 服务层的 token-to-session 兑换动词）/ `Adopt`（用户接纳一条 Proposal）/ `Invalidate`（base_path 失效）
- 状态机词汇：Worker `online` / `offline`；BootstrapToken `active` / `used` / `expired` / `revoked`；WorkerProjectProposal `pending` / `accepted` / `ignored` / `superseded`
...
| 命名冲突 | accept 时 suggested_project_id 已存在 → 用户 UI 提示让用户改 |
...
### 3.6 MappingInvalidationService
...
| Center 行为 | mapping.status = invalidated（不删，保留血缘）；用户 UI 提示用户决定是否重新映射；不自动迁移 |
...
**唯一 caller**：ProposalDiscoveryService（§ 3.4）。Worker 端 scan 发现新 candidate 时，跨网络通知 Center 建 Proposal。

### 4.4 ProjectFactory

**两个 caller**：
- ProposalReviewService（§ 3.5）：accept Proposal 时若 project 不存在则同事务建
...
**两个 caller**：
- ProposalReviewService（§ 3.5）：accept 时同事务建
...
### 5.2 BootstrapTokenRepository（sub-repo of Worker, [ADR-0023](../../../decisions/0023-worker-enroll-lightweight.md)）
...
### 5.3 WorkerProjectMappingRepository（sub-repo of Worker）
...
### 5.4 WorkerProjectProposalRepository
...
| `worker_project_proposal.proposed` | 推用户 UI（Web Console / CLI）让用户决策 accept / ignore / 改后 accept |
| `worker_project_mapping.invalidated` | 用户 UI 提示用户决定是否重新映射 |
| `project.created` / `project.removed` | 一般不直接动作（除非有等候该 project 的 open task）|

详见 [cognition/00-overview.md](../cognition/00-overview.md)。

### 7.2 Observability 订阅
...
- [ADR-0024 AgentInstance 一等公民化（草案）](../../../decisions/0024-agent-instance-first-class.md)（**historical** — 已被 Agent BC ADR-0049 supersede）
...
- [conversation/00-overview.md](../conversation/00-overview.md) — Identity / ChannelBinding（用户跟渠道的关联，跟 Worker 是不同维度）
