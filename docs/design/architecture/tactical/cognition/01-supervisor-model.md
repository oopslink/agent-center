...
唤醒事件清单的权威来源 = [task-runtime/00-overview § 7.1 Supervisor 唤醒事件白名单](../task-runtime/00-overview.md) + 本节补充。**`supervisor.*` 事件不在白名单**，避免自唤醒循环。
...
Supervisor 用同样的 `inspect` / `query` / `ps` CLI 查 task / execution / issue / worker / conversation / 等。**不为 supervisor 单造 RPC**（[task-runtime/00-overview § 7 跨 BC 交互](../task-runtime/00-overview.md) / [observability § O5](../observability/01-observability.md)）。
...
- [03 限界上下文 § BC4 Cognition](../../strategic/03-bounded-contexts.md)（注：[ADR-0019](../../../decisions/0019-bc-scheduling-execution-merged-to-task-runtime.md) 后编号 BC5 → BC4）
- [TaskRuntime BC § 7.1 唤醒事件白名单](../task-runtime/00-overview.md)
