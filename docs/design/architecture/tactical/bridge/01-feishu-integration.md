...
| 触发 | 处理 |
|---|---|
| CLI `task bind-conversation <task_id> --channel=feishu --to=<conv_id>` | center 直接绑既有 conversation；Bridge 不动作 |
| CLI `task bind-conversation <task_id> --channel=feishu --auto` | center 新建 kind=task Conversation + emit `conversation.opened` → Bridge 发 root card 到 `notification.default_channel`（或 CLI 指定的渠道）|
| 飞书 slash `/track <task_id>` | Bridge 翻译为 `task bind-conversation --channel=feishu --to=<当前 thread 对应 conversation>`（详见 § 9.1）|
| 飞书 @bot 自由文本 "盯一下 T-42" | Bridge 写 inbound Message → supervisor wake → 解析意图 → 调 `task bind-conversation`（烧 LLM；兜底用）|
...
**Issue**（同模板，CLI 命令名 `task bind-conversation` → `issue bind-conversation`；v1 不引入 InputRequest fallback 等价物）：
...
| `/track <task_id>` | Bridge 解析 → 翻译为 `task bind-conversation --channel=feishu --to=<当前 thread 对应 conversation_id>`（懒创建路径之一，见 § 7.5.2）；若当前 thread 还没对应 conversation，先按 § 4 inbound 流程创建 dm / group_thread conversation 再绑 | center 写 task.conversation_id + emit conversation 关联记录；supervisor 唤醒后主动写一条 supervisor_summary 到 task.conversation_id 提示绑定成功并续后续进度 |
