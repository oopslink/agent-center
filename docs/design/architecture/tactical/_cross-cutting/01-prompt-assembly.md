...
2. 如 envelope 带 `extra_skill_files`，按列表拼上去
3. 按"层次"顺序组合：`worker-agent.md skill + constraints_extra(若有) + task_description (或从 blob_ref 取)` → `final_prompt`
4. `cd` 到 worktree 路径（`workspace_mode=worktree` 时为 `base_path + ".wt/task-<execution_id>"`；`workspace_mode=direct` 时为 `base_path`）
