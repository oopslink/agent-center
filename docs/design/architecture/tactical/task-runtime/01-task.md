...
| `conversation_id` | uuid | 可空 | 该 task 绑定的 `kind=task` Conversation；按来源决定创建时机（a/e 同步建 / b/c/d 懒创建 null）；详见 § 7 与 [ADR-0017](../../../decisions/drafts/0039-conversation-business-model-v2-unified.md) |
...
[ADR-0017](../../../decisions/drafts/0039-conversation-business-model-v2-unified.md)：**Task ↔ Conversation 1:1**。所有 task 相关可见 IO（supervisor 分析 / worker 进展 / agent 请示 / 用户回应）走同一个 `kind=task` Conversation，复用既有 Message 模型。
...
详见 [ADR-0017 § 10](../../../decisions/drafts/0039-conversation-business-model-v2-unified.md)。
