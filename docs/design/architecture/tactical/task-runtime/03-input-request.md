...
由 [ADR-0017 (superseded by 0039) § 6](../../../decisions/0039-conversation-business-model-v2-unified.md) 提前到 v1（~~飞书 D2 slash 模式首批命令之一，详见 `tactical/bridge/01-feishu-integration.md § 9` (v2 deleted per ADR-0031)~~）。~~Bridge 收 slash → 直接调 `InputRequest.respond` + 写一条 `text` Message 留痕 → **不经 supervisor**。~~ (v2 删 vendor per ADR-0031)
...
- **Center 硬规则 fallback**：自动 bind 到 `notification.default_channel` 配置项指定的渠道（详见 [ADR-0017 § 10.4](../../../decisions/0039-conversation-business-model-v2-unified.md)）
