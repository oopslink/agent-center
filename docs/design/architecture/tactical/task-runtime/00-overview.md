...
- **Bridge → TaskRuntime**：Customer-Supplier（inbound 时 Bridge 调 `task bind-card` / `InputRequest.respond`）
...
|---|---|---|---|---|
| **Task → Conversation**（`task.conversation_id`） | 强 / 1:1 | tx 同步（a/e 路径同事务建；b/c/d 路径懒创建另事务） | task 创建（a/e）/ task bind-card 命令（b/c/d） | [ADR-0017](../../../decisions/0017-task-as-conversation.md) |
...
Observability BC 订阅 TaskRuntime 全部事件做投影 / 查询 / inspect。详见 [observability/01-observability.md § 事件总览](../observability/01-observability.md)。
