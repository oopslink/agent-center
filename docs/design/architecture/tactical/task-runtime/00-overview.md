...
- **TaskRuntime ↔ Conversation**：Shared Kernel（`task.conversation_id` 1:1 强引用 `kind=task` Conversation；同事务双写，[ADR-0017 (superseded by 0039)](../../../decisions/0039-conversation-business-model-v2-unified.md)）
...
- ~~**Bridge → TaskRuntime**：Customer-Supplier（inbound 时 Bridge 调 `task bind-conversation` / `InputRequest.respond`）~~ (v2 删 per ADR-0031)
...
| 维度 | 内容 |
|---|---|
| 入参 | `task_id`（必）+ `agent_instance_id`（必，[ADR-0024](../../../decisions/0024-agent-instance-first-class.md)）+ workspace_mode + 等；worker_id 通过 AgentInstance 拿 |
...
| ~~**Web Console**~~ | ~~用户 Web UI 新建~~ | ~~task.created 同事务建 Conversation + Bridge 发 Task root card~~ (v2 删 per ADR-0031) |
| ~~**Bridge**（飞书 @bot）~~| ~~用户飞书 @bot 直接发起~~ | ~~task.created 同事务建 Conversation + Bridge 发 Task root card 回写 primary_channel_thread_key~~ (v2 删 per ADR-0031) |
...
|---|---|---|---|---|
| **Task → Conversation**（`task.conversation_id`） | 强 / 1:1 | tx 同步（a/e 路径同事务建；b/c/d 路径懒创建另事务） | task 创建（a/e）/ task bind-conversation 命令（b/c/d） | [ADR-0017](../../../decisions/0039-conversation-business-model-v2-unified.md) |
...
| `input_request.requested` | ~~推飞书卡片~~ / 提议答案 (v2 删 vendor per ADR-0031) |
...
### 7.2 ~~Bridge 渲染（outbound）~~ (v2 删 per ADR-0031)
...
~~详见 `tactical/bridge/01-feishu-integration.md`~~ (v2 deleted per ADR-0031)
...
| TaskRuntime ↔ Workforce | Shared Kernel | Task / TaskExecution 引用 worker_id / project_id |
| TaskRuntime ↔ Conversation | Shared Kernel | task.conversation_id 1:1（[ADR-0017 (superseded by 0039)](../../../decisions/0039-conversation-business-model-v2-unified.md)） |
| ~~Bridge → TaskRuntime~~ | ~~Customer-Supplier~~ | ~~inbound 时 Bridge 调 `task bind-conversation` / `InputRequest.respond`~~ (v2 删 per ADR-0031) |
...
| Task ↔ Conversation 多 tracker（1:N，多用户多渠道镜像）| [roadmap](../../../roadmap.md)（[ADR-0017 § 9](../../../decisions/0039-conversation-business-model-v2-unified.md) 显式驳回 v1）|
...
- ~~tactical/bridge/01-feishu-integration.md (v2 deleted per ADR-0031)~~
