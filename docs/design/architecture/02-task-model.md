...
| `working → input_required` | `input_request.requested` | Worker daemon（代 agent） | 详见 [04-input-required.md](04-input-required.md) |
...
| 3 | `input_request_timeout` | InputRequest 在 pending | T1=4h ping / T2=24h fail（见 [04-input-required.md](04-input-required.md)） | InputRequest → timed_out → execution → failed(`input_timeout`) |
...
详见 [05-observability.md](05-observability.md) 与实现层 schema。
...
详细 conclude 流程归 [03-issue-discussion.md](03-issue-discussion.md)。本节只列对 Task 的影响。
...
详细 supervisor 模型见 [06-supervisor-model.md](06-supervisor-model.md)。
...
详见 [05-observability.md](05-observability.md) + supervisor.md skill。
