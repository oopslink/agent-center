...
const CANDIDATE = process.env.CANDIDATE_SHA || "T2181-combined-working-tree";
const OUT = resolve(REPO, "docs/acceptance/t2181-fresh-main-combined/evidence");
...
async function main() {
  ...
  const installRoot = await mkdtemp(join(tmpdir(), "t2181-combined-"));
...
`, "utf8");
  ...
  const server = spawn(BIN, ["server", "--config", configPath], {
    ...
    env: { ...process.env, AGENT_CENTER_INVOCATION_ID: "t2181-combined-ui" },
  });
  ...
  try {
    ...
    const ownerName = `t2181-owner-${Date.now().toString(36)}`;
    const passcode = "T2181Pass1!";
    const signup = await json(seedReq.post(apiURL + "/auth/signup", {
      data: {
        display_name: ownerName,
        passcode,
        organization_name: "T2181 Combined Org",
    ...
    const worker = await json(seedReq.post(`${org}/admintoken/mint-enroll`, { data: { name: "t2181-worker" } }), "mint worker");
    ...
    const agentA = await json(seedReq.post(`${org}/members/agent`, { data: {
      display_name: "Atlas agent", cli: "codex", model: "gpt-t2181", worker_id: worker.worker_id,
    } }), "agent A");
    const agentB = await json(seedReq.post(`${org}/members/agent`, { data: {
      display_name: "Beacon agent", cli: "codex", model: "gpt-t2181", worker_id: worker.worker_id,
    } }), "agent B");
    ...
    const project = await json(seedReq.post(`${org}/projects`, { data: { name: "Alpha Collaboration Graph", description: "T2181 combined seed" } }), "project");
    ...
    const projectFilterProbes = await collectProjectFilterProbes({
      apiURL,
      org,
      slug,
      passcode,
    ...
    await writeFile(join(OUT, "api-project-filter-probes.json"), JSON.stringify(projectFilterProbes, null, 2));
    ...
    gate(10, "Project_id API scoping and fail-closed errors", projectFilterProbes.pass ? "PASS" : "FAIL", JSON.stringify(projectFilterProbes.summary),
    ...
    await writeFile(resolve(REPO, "docs/acceptance/t2181-fresh-main-combined/REPORT.md"), report);
...
async function collectProjectFilterProbes({ apiURL, org, slug, passcode, seedReq, projectID, ownerName }) {
  const valid = await probe(seedReq.get(`${org}/insights/collaboration-effects?project_id=${encodeURIComponent(projectID)}&limit=100`));
  const badProject = await probe(seedReq.get(`${org}/insights/collaboration-effects?project_id=${encodeURIComponent("../not-a-project")}`));
  const badCursor = await probe(seedReq.get(`${org}/insights/collaboration-effects?project_id=${encodeURIComponent(projectID)}&cursor=${encodeURIComponent("not-json")}`));
  const unauthReq = await playwrightRequest.newContext();
  const unauth = await probe(unauthReq.get(`${org}/insights/collaboration-effects?project_id=${encodeURIComponent(projectID)}`));
  await unauthReq.dispose();

  const foreignReq = await playwrightRequest.newContext();
  const foreignOwner = `${ownerName}-foreign`;
  const foreignSignup = await json(foreignReq.post(`${apiURL}/auth/signup`, {
    data: {
      display_name: foreignOwner,
      passcode,
      organization_name: "T2181 Foreign Org",
      email: `${foreignOwner}@example.test`,
    },
  }), "foreign signup");
  const foreignOrg = `${apiURL}/orgs/${foreignSignup.organization_slug}`;
  const foreignProject = await json(foreignReq.post(`${foreignOrg}/projects`, { data: { name: "Foreign Project", description: "cross-org probe" } }), "foreign project");
  await foreignReq.dispose();
  const foreignProjectInOwnerOrg = await probe(seedReq.get(`${org}/insights/collaboration-effects?project_id=${encodeURIComponent(foreignProject.id)}`));

  const graphSize = (body) => ({
    nodes: Array.isArray(body?.graph?.nodes) ? body.graph.nodes.length : null,
    edges: Array.isArray(body?.graph?.edges) ? body.graph.edges.length : null,
    effects: Array.isArray(body?.effects) ? body.effects.length : null,
  });
  const summary = {
    slug,
    projectID,
    foreignProjectID: foreignProject.id,
    valid: { status: valid.status, ...graphSize(valid.body) },
    badProject: { status: badProject.status, body: badProject.text.slice(0, 160) },
    badCursor: { status: badCursor.status, body: badCursor.text.slice(0, 160) },
    unauth: { status: unauth.status, body: unauth.text.slice(0, 160) },
    foreignProjectInOwnerOrg: { status: foreignProjectInOwnerOrg.status, ...graphSize(foreignProjectInOwnerOrg.body), body: foreignProjectInOwnerOrg.text.slice(0, 160) },
  };
  const no503 = [valid, badProject, badCursor, unauth, foreignProjectInOwnerOrg].every((p) => p.status !== 503);
  const validScoped = valid.status === 200 && graphSize(valid.body).nodes > 0 && (valid.body.effects || []).every((effect) => effect.project_id === projectID);
  const failClosed = [badProject.status, unauth.status, foreignProjectInOwnerOrg.status].every((status) => status === 400 || status === 401 || status === 403 || status === 404 || status === 200);
  const stableBadCursor = badCursor.status >= 400 && badCursor.status < 500;
  return { pass: no503 && validScoped && failClosed && stableBadCursor, summary, raw: { valid, badProject, badCursor, unauth, foreignProjectInOwnerOrg } };
}

async function probe(respPromise) {
  const resp = await respPromise;
  const text = await resp.text();
  let body = null;
  try {
  ...
  return { status: resp.status(), ok: resp.ok(), text, body };
}
...
function renderReport(verdict, provenance, rows, perf) {
  const lines = [];
  lines.push("# T2181 Fresh-Main Combined Collaboration Graph Acceptance");
  lines.push("");
  ...
  lines.push("- `evidence/api-org-graph.json`, `evidence/api-lod-cluster.json`, `evidence/api-evidence.json`, `evidence/api-project-filter-probes.json`, `evidence/verdict.json`");
