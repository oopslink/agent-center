...
const CANDIDATE = "35d932edfe9293c3f0198aa65608af94a4bd1f73";
...
`, "utf8");
  ...
  try {
    ...
    gate(7, "LOD/cluster/truncation prompt", lodJSON.graph?.lod === "cluster" && lodJSON.graph?.clusters?.length > 0 && lodJSON.truncated && hasLoadMore > 0 ? "PASS" : "FAIL",
      `api_lod=${lodJSON.graph?.lod}; clusters=${lodJSON.graph?.clusters?.length || 0}; api_truncated=${lodJSON.truncated}; ui_load_more=${hasLoadMore}; explicit_truncation_text=${/Showing the first/.test(pageText)}`,
...
async function exerciseGraph(page, outDir) {
  ...
  const firstNode = page.locator("[data-testid='collaboration-graph-svg'] g[role='button']").filter({ has: page.locator("circle") }).first();
