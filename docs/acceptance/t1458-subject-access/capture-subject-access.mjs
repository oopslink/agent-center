...
async function routeJSON(page) {
  await page.route('**/*', (route) => {
    ...
    if (pathname === '/api/sse') return route.fulfill({ status: 200, contentType: 'text/event-stream', body: ': ok\n\n' });
  ...
  await page.route('**/api/orgs/test/projects**', (route) => route.fulfill({ json: { projects: [] } }));
...
async function main() {
  ...
  const evidence = {
    viewport_1684: { width: 1684, height: 934 },
    console_errors: [],
    request_failed: [],
    http_errors: [],
    expected_http_conflicts: [],
    overflow_1280: null,
  };
  ...
  page.on('requestfailed', (request) => {
    evidence.request_failed.push({ url: request.url(), error: request.failure()?.errorText || '' });
  });
  page.on('response', (response) => {
    if (response.status() === 409 && response.url().includes('/access/batch/apply')) {
      evidence.expected_http_conflicts.push({ url: response.url(), status: response.status() });
      return;
    }
    if (response.status() >= 400) evidence.http_errors.push({ url: response.url(), status: response.status() });
  });
  ...
  await page.getByTestId('access-subject-row-agent:builder').click();
  evidence.overflow_1280 = await page.evaluate(() => {
    const doc = document.documentElement;
    const selectors = [
      'page-Access',
      'access-subject-view',
      'access-subject-detail',
      'access-subject-metrics',
      'access-decision-table',
      'access-permission-trace',
      'access-direct-binding-union',
    ];
    const elements = Object.fromEntries(selectors.map((id) => {
  ...
  await page.setViewportSize({ width: 1684, height: 934 });
  ...
  await fs.writeFile(path.join(outDir, 'capture-state.json'), `${JSON.stringify(evidence, null, 2)}\n`);
  await browser.close();
}
