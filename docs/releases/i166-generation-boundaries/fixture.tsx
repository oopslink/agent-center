...
tasks.push({task_id:'t2216',org_ref:'T2216',title:'I166 原发布核验（已取消）',assignee_ref:'agent:agent-center-pd',task_status:'discarded',node_status:'discarded',depends_on:[],effective:false});
const generationOf = {t2216:0,t2215:0,t2213:0,t2214:0,t2217:1,t2223:3};
const snapshots = [0,1,2,3].map(revision=>({id:`gen-${revision}`,revision,reason:`第 ${revision+1} 代：交付与整改`,evidence:'Fixture',created_at:`2026-09-${15+revision}T01:00:00Z`,snapshot:{tasks:tasks.filter(t=>generationOf[t.task_id]<=revision).map(t=>({...t,status:t.task_status})),edges:tasks.flatMap(t=>t.depends_on.map(to_task_id=>({from_task_id:t.task_id,to_task_id,kind:'seq'}))),dispatch_records:tasks.map(t=>({task_id:t.task_id}))},snapshot_progress:{done:2,total:4}}));
snapshots.forEach((generation, revision) => {
 generation.diff = {tasks:[], edges:[], node_decisions: revision === 2 ? [{task_id:'t2215',action:'supersede',reason:'旧候选验收未通过'}, {task_id:'t2216',action:'supersede',reason:'旧候选验收未通过'}] : []};
 generation.parent_generation_id = revision ? `gen-${revision-1}` : '';
 if (revision < 2) {
  generation.snapshot.tasks.forEach(task => { if (['t2215','t2216'].includes(task.task_id)) task.status = 'open'; });
  generation.snapshot.edges.push({from_task_id:'t2215',to_task_id:'t2214',kind:'seq'}, {from_task_id:'t2216',to_task_id:'t2215',kind:'seq'});
 }
});
...
window.fetch=async (input,init)=>{
 ...
 if(url.endsWith('/generations'))data={plan_id:'review',active_generation_id:'gen-3',generations:snapshots,nodes:tasks.map(t=>({task_id:t.task_id,revision:generationOf[t.task_id]}))};
