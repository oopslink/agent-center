...
**② Secondary nav drawer** — tapping the top-bar "Workspace" button opens a `BottomSheet` sliding up from the bottom with a drag handle, a "WORKSPACE" section header, and links for Projects (highlighted active) / Issues / Tasks / Plans, plus additional Workspace-module links (Repos / Model catalog) beyond the mockup's 4-item illustrative list. The prior Templates entry was retired in the 2026-08 Team Memory rules closeout. Verified interaction: clicking "Tasks" navigated to `/organizations/<org>/tasks` and the sheet auto-closed (matches the existing `AppLayout.mobilenav.test.tsx` coverage cited in the plan).
