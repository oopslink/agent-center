...
> **三层分支流（owner 2026-06-16 定）**：**开发分支 `dev/<ver>-*`** —(integration-dev 开发期合并)→ **迭代主干分支（如 `v2.10.3`，独立于 `main`）** —(PD ship 合并 + tag)→ **`main`**。
> - **integration-dev 唯一职责** = 开发期把各 accepted 开发分支合并进**迭代主干**（唯一机械合并人）；**不参与验收、不参与 ship、不碰 `main`**。
> - **合并到 `main` 是 ship 操作，只有 PD 做**，且**只从迭代主干合**——**PD 绝不直接把 dev feature 分支合进 `main`**。

整条「验收 → ship」由 **PD 跑完**（dev 分支 → 迭代主干的集成除外——那是 integration-dev 开发期的活）。固定五步：
...
4. **Owner 同意验收**。
5. **PD 独立完成 ship**（前提：本周期 dev 分支已由 integration-dev 集成进迭代主干 `<version>`；顺序）：① 更新 **README + CHANGELOG + sites**（版本页 / 当前指针）；② **构建硬门**（§ 8：`make lint` 含 `lint-spa-tsc` + `make build` 绿）；③ **从迭代主干 `<version>` 合并进 `main`**（在**干净的临时 worktree**，**不碰**带未提交 WIP 的部署 worktree；verify 无冲突 + 构建绿再推）；④ **打 tag `<version>`** 并推送；⑤ **清理迭代主干 + 所有开发分支**（远端 + 本地 `dev/<ver>-*`、`tester/<ver>-*` + worktree，**保留 tag**），列出已删分支清单。
   - **职责红线**：dev 分支 → 迭代主干 = **integration-dev**（开发期）；迭代主干 → `main` + tag = **PD ship**。**PD 绝不直接把 dev feature 分支合进 `main`**。若 ship 时迭代主干缺失、或有 accepted 但未集成进迭代主干的 dev 分支，PD **先请 integration-dev 集成进迭代主干**，再继续 ship。
...
**§ 11 自检：** 验收项是否逐任务（数量=任务数）？每项有 AFTER 效果图且已 commit 仓库？报告是核真实 verdict 还是凭节点状态？**dev 分支 → 迭代主干是否由 integration-dev 集成（PD 没直接把 dev 分支合进 main）**？ship（README+CHANGELOG+sites / 构建门 / **迭代主干→main** / tag / 清分支）是否 PD 独立跑完？
