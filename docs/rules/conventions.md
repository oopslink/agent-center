...
> v2 撤回所有 vendor 集成（per [ADR-0031](../design/decisions/drafts/0031-v2-drop-bridge-vendor-integration.md)）；v2 用户主入口 = Web Console + CLI。本节规则仍有效作为通用原则（领域模块零外部依赖）；具体外部接入 v3+ 才重新设计。
...
| ~~Feishu ↔ Center 认证~~ | v2 已撤回（per [ADR-0031](../design/decisions/drafts/0031-v2-drop-bridge-vendor-integration.md)） |
