...
> 🆕 **v2 状态**（[ADR-0031](../design/decisions/drafts/0031-v2-drop-bridge-vendor-integration.md)）：v2 撤回所有 vendor 集成（含飞书 + Bridge BC）；用户主入口 = Web Console + CLI。**本节规则仍有效**（领域模块零 vendor 依赖原则不变）但 v2 没具体 vendor 接入；v3+ 重新设计 Bridge / vendor view layer 时复活。

领域模块（Issue / Conversation / Task / 等）是**系统内部模块**，**不直接调用**任何 vendor 的 SDK 或 API。

外部集成（v3+）通过独立的 **Bridge 程序**实现：

- **Outbound（系统 → vendor）**：Bridge 订阅领域事件 → 推到 vendor
- **Inbound（vendor → 系统）**：Bridge 收 vendor 回调 → 写到领域模块的 API
- 每个 vendor 一个 Bridge 实现

这样领域模块零 vendor 依赖，可独立测试、可独立运行（无 Bridge 时系统仍跑，只是没人能从 vendor 侧 IO）。

**自检：** 我的领域模块代码里 import 了 vendor SDK 吗？把它移到 bridge 包里（v2 = 直接撤掉；v3+ 待 Bridge 重设计后）。
...
| 项 | 当前要求 |
|---|---|
| Worker ↔ Center 认证 | bootstrap token + session token（v2: BootstrapToken Entity stateful，[ADR-0023](../design/decisions/drafts/0023-worker-enroll-lightweight.md)） |
...
| **用户密钥**（MCP env vars / 云凭据等） | v2: 中心化在 [SecretManagement BC](../design/architecture/tactical/secret-management/00-overview.md) 管理（[ADR-0026](../design/decisions/drafts/0026-user-secret-management-bc.md)）；AES-GCM 加密；master key 配置文件（不入 DB）；CLI 不打印明文；引用语法 `secret:<name>` |
