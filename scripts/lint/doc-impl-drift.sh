...
check_handlers_no_persistence_open() {
  local whitelist_re='handlers_(migrate_v1_to_v2|migrate_ai_runtime|system)\.go'
  local hits
  set +e
  ...
  handlers_migrate_v1_to_v2.go + handlers_migrate_ai_runtime.go +
  handlers_system.go (the actual server boot path). These files violate the rule:
$offenders
