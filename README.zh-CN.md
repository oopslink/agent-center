...
> [!IMPORTANT]
> **v2.10.1 已发布（2026-06-15）** —— **移动端体验 + 开放认领 + 桌面增强。** v2.10.0 三栏桌面骨架现已适配到 <768 **移动布局**:**底部 Tab 导航**(Workspace / Conversations / Members / System)驱动 整屏列表 → 整屏详情 → 底部 sheet 的层级流;Plan 的 DAG 转为**纵向 stepper**,Work Board 竖屏横滑列 / 横屏铺开,Members / System 也一并适配。**Claimability(ADR-0047)**:backlog 任务仍不可领,但 **Assignment Pool 开放认领** —— 任意项目成员可领(fail-closed 授权,对非成员 opaque)、**每 agent 持有上限 N=3**、原子 CAS 认领(无双 assignee)。**桌面增强**:Thread / 参与者侧栏可**拖拽调宽**(共用 `ResizablePanel`,持久化)、Channel 侧栏新增 **Chat / Threads / Files** 三 tab、全局 Plan 列表可查看 **Archived 计划(只读)**、col① 侧栏整合连接状态 / 搜索 / 主题切换 / Sign out。外加:**Plan `P<number>` 序列号** + 消息 `plan-id` / `P123` linkify(与 `T<number>` 对称)、列表 `org_ref`(`T<n>`)全面取代 `#短哈希`、Task / Issue 侧栏 agent 名可点击弹出活动侧栏、Task → 关联 Plan 跳转、**入站附件现在 agent 可见**(wake 透传 `file_uri`,fail-closed)、SSE 连接更稳(`no-transform`)、以及 `discard_task` agent 工具。详见 [CHANGELOG](./CHANGELOG.md)。
>
...
<p align="center">
  <img src="./assets/screenshots/web-console-work-board.png" alt="Web Console —— 项目 Work Board:Backlog、认领池、结构化 Plan 三段" width="820">
  <br/>
  <em>Work Board —— 项目任务的三段视图:Backlog · 认领池(开放认领)· 结构化 Plan。</em>
</p>
