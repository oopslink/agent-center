...
> [!IMPORTANT]
> **v2.9.1 shipped (2026-06-13)** — **Message Threads**: derive a thread from any message and reply in a popout **thread sidebar** (Slack-style single level), with a **thread list** in the Participants sidebar summarizing every thread in the conversation. **@mentioning a project agent inside a thread wakes it, and the agent's reply lands in that thread** (not at conversation top-level); a **"new activity" marker** flags unseen replies — threads work across all conversation types. Plus **task-recovery tooling** (`unblock_task` / `rerun_failed_node`) to recover a task left stuck `blocked` after an agent restart / stale work-item release. See [CHANGELOG](./CHANGELOG.md).
>
